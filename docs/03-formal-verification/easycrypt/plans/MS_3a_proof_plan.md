...
| `A_ms3a_real_seed_bits_programmed_on_support`, `A_ms3a_real_seed_bitness_globals_programmed_on_support` | **Axioms** (`SourceObligations.ec`) — real seed support: `ms_per_bit_programmed` and `ms_ordered_challenge_vector_matches` (`BitnessVector.ec`), the two conjuncts of `ms_bitness_vector_programmed_layer` |
| `A_ms3a_real_seed_programmed_on_support` | **Proved lemma** (`SourceObligations.ec`) — former real programmed-on-support axiom, from the two axioms above + unfolds |
...
| `A_ms3a_sim_seed_bits_programmed_on_support`, `A_ms3a_sim_seed_bitness_globals_programmed_on_support` | **Axioms** (`SourceObligations.ec`) — sim seed support (keyed by `s`): same two conjuncts as real |
| `A_ms3a_sim_seed_programmed_on_support` | **Proved lemma** (`SourceObligations.ec`) — former sim programmed-on-support axiom, from the two sim axioms above + unfolds |
...
| `A_ms3a_seed_pair_stmt_source_shared` | **Axiom** (`SourceObligations.ec`) — joint seed support: real/sim seed **record** statement digests agree (shared public statement source at sampling) |
| `A_ms3a_seed_pair_stmt_on_support` | **Proved lemma** (`SourceObligations.ec`) — payload `from_seed` stmt equality, from `A_ms3a_seed_pair_stmt_source_shared` + identity |
| `A_ms3a_seed_pair_res_source_shared` | **Axiom** (`SourceObligations.ec`) — joint seed support: real/sim seed **record** result bits agree |
| `A_ms3a_seed_pair_res_on_support` | **Proved lemma** (`SourceObligations.ec`) — payload `from_seed` res equality, from `A_ms3a_seed_pair_res_source_shared` + identity |
| `A_ms3a_seed_pair_comparison_global_source_shared` | **Axiom** (`SourceObligations.ec`) — joint seed support: real/sim seed **record** comparison-global digests agree |
| `A_ms3a_seed_pair_comparison_global_on_support` | **Proved lemma** (`SourceObligations.ec`) — payload `from_seed` comparison-global equality, from source-shared + identity |
| `A_ms3a_seed_pair_bitness_globals_on_support` | **Axiom** (`SourceObligations.ec`) — last public-field conjunct of `ms3a_payload_pair_public_fields_match` on joint seed support |
| `A_ms3a_seed_pair_public_fields_on_support` | **Proved lemma** (`SourceObligations.ec`) — combines stmt/res/comparison-global lemmata + bitness-globals axiom |
...
| Seed support (programmed layer on **seed** support) | **Lemmas** `A_ms3a_real_seed_programmed_on_support`, `A_ms3a_sim_seed_programmed_on_support` from four **axioms** (`*_real_*` / `*_sim_*` bits + bitness-globals on support; `ms/source/SourceObligations.ec`) — quantify over `d_ms3a_{real,sim}_payload_seed` only |
| Seed support (paired **public** fields on joint seed support) | **Axioms** `A_ms3a_seed_pair_stmt_source_shared`, `A_ms3a_seed_pair_res_source_shared`, `A_ms3a_seed_pair_comparison_global_source_shared`, `A_ms3a_seed_pair_bitness_globals_on_support`; **lemmata** `A_ms3a_seed_pair_stmt_on_support`, `A_ms3a_seed_pair_res_on_support`, `A_ms3a_seed_pair_comparison_global_on_support` (`ms/source/SourceObligations.ec`); **lemma** `A_ms3a_seed_pair_public_fields_on_support` combines into `ms3a_payload_pair_public_fields_match` for `from_seed` |
| Payload support (programmed layer on payload support) | **Proved lemmas** `ms3a_payload_real_support_programmed`, `ms3a_payload_sim_support_programmed` (`ms/source/SourceObligations.ec`) — from real/sim programmed lemmas + defining `dmap` + `supp_dmap` |
...
- Split the old “one-shot” source equality story: the **schedule** is now axiom **`A_ms3a_payload_dmap_bitness_layer_schedule`** (unconditional payload `dmap` equality — core coupling target). The former `ms3a_ax_*` premises are **proved lemmas** unpacking support from the three **payload-support lemmas**, which in turn follow from **seed-support** (real/sim programmed lemmas each from two field axioms + four source-shared / bitness-globals axioms + lemmata `A_ms3a_seed_pair_stmt_on_support`, `A_ms3a_seed_pair_res_on_support`, `A_ms3a_seed_pair_comparison_global_on_support`, `A_ms3a_seed_pair_public_fields_on_support`) and the defining payload `dmap`s; **`ms3a_payload_schedule_equivalence`** is **deprecated / compatibility** packaging (ignores those hypotheses). **`ms3a_source_eq_from_bitness_layer`** unfolds folded `d_ms3a_bitness_*_source` and applies the schedule axiom directly.
