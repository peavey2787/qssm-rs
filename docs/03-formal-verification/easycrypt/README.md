...
- The public semantic rejection budget is now grounded in a ticket-failure law rather than described only as a demo branch mass: `BudgetParameters.epsilon_le_rej_semantic` is the primitive semantic ticket-failure probability, `LERealExecution.le_real_execution_semantic_rejection_ticket_failure_probability` proves the concrete execution-owned ticket sampler matches that law, and `LERejectionSampler.le_rejection_shadow_semantic_failure_probability` is now proved equal to that ticket-failure quantity. Its current value is still `1%r / 4%r`, but the theorem-facing budget is now expressed through the ticket/repair semantics. The next meaningful modeling refinement is still not a parameter-owner move; with both semantic count owners frozen in `BudgetParameters.ec`, the next local target is to enrich the current primitive `1%r / 4%r` ticket-failure law beyond the one-repair-slot surrogate without adding axioms.