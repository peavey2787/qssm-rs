...
- **`ms3c_ax_payload_public_fields_match`**, **`ms3c_ax_payload_challenge_shares_match`**, **`ms3c_ax_payload_challenge_share_consistency`**, **`ms3c_ax_payload_false_clauses_simulated`**, **`ms3c_ax_payload_true_clause_simulated`** — these five are the explicit antecedents of **`A_ms3c_coupling_pair_relation`**.
...
| **`A_ms3c_coupling_pair_relation`** | axiom | Five facts (**`ms3c_ax_payload_public_fields_match`**, **`ms3c_ax_payload_challenge_shares_match`**, **`ms3c_ax_payload_challenge_share_consistency`**, **`ms3c_ax_payload_false_clauses_simulated`**, **`ms3c_ax_payload_true_clause_simulated`**) ⇒ **`ms3c_ax_payload_coupling_pair_relation`** (no **`ms3c_ax_payload_announcements_match_shape`** or **`ms3c_ax_payload_announcement_digests_preserved`** premises; both follow from public fields / unconditional shape lemmas; quantified only for **`(pr, ps)`** on the joint support; **not** implied by the independent product). |
