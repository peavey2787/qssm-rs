...
Current status: the tree checks cleanly with `./check_easycrypt.sh`; the current checker snapshot is `OK` over 126 checked theories; `axiom_count=0`; `admit_count=0`; the exact-zero route is unchanged; and the live demo semantic route still closes at `qssm_main_theorem_semantic_budget = 3%r / 4%r`. That is a machine-checked statement about the current abstraction boundary, not yet a production-count cryptographic security estimate for the deployed system.

Freeze checkpoint, May 2026: theorem-route implementation is intentionally stopped at a three-lane split.
...
- The MS parameterized route is checker-green only as a staged/public-endpoint lane through `MSProbabilitySurfaceParameterized.ec`, `GameAdvantageParameterized.ec`, `GameMSHopTypesParameterized.ec`, and `GameMSHopCompositionParameterized.ec`. It is not routed into the canonical `G0 -> G1` theorem surface.
- The LE parameterized route is checker-green all the way to the theorem layer through `GameLEBridgeParameterized.ec` and `theorem/MainTheoremParameterized.ec`, where `qssm_main_theorem_le_parameterized_budget` keeps the canonical/demo MS contribution unchanged and parameterizes only the LE side.

`qssm_main_theorem_parameterized_budget` does not currently exist, and that is intentional. The blocker is semantic, not organizational: the public AfterRom observable is only budget-close to the canonical AfterRom observable, not zero-equal, so there is no zero-cost `public AfterRom -> canonical AfterRom` landing theorem on the current lower MS surface. The canonical `G0 -> G1` MS parameterized route therefore remains intentionally unstated.
...
- The EasyCrypt tree is checker-green today: `./check_easycrypt.sh` currently reports `OK` over 126 checked theories.
- Repo-local named axioms in `*.ec` files under this directory are currently at `0`, and the current repo-local `admit` count is `0`.
- The theorem split is intentionally frozen: the canonical/demo route remains live, the MS parameterized route remains staged/public-endpoint only, and `qssm_main_theorem_le_parameterized_budget` is the only top-level parameterized theorem currently claimed.
- A full canonical `qssm_main_theorem_parameterized_budget` is intentionally unclaimed because the current MS public AfterRom endpoint is only budget-close to the canonical AfterRom endpoint, not zero-equal.
