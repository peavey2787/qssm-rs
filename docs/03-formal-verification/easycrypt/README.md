...
47. `ms/source/SourceScheduleSeed.ec` (lemma `A_ms3a_bitness_layer_seed_schedule` + `L_ms3a_bitness_layer_seed_schedule_composed_form`)
48. `ms/source/SourceSchedulePayload.ec` (payload `dmap` schedule, `ms3a_ax_*` from payload support, `ms3a_payload_schedule_equivalence`)
49. `ms/source/SourceScheduleTheorem.ec` (`ms3a_source_eq_from_bitness_layer`, constructor-scoped lemmas)
50. `ms/source/SourceScheduleObligations.ec` (facade: `require export` of the three schedule modules)
51. `ms/source/SourceObligations.ec` (facade)
52. `ms/source/SourceTheorem.ec`
53. `ms/MS.ec`
54. `le/LESurface.ec`
55. `le/LESetB.ec`
56. `le/LERejection.ec`
57. `le/LEFsProgramming.ec`
58. `le/LEViewIndist.ec`
59. `le/LEStatisticalDistance.ec`
60. `le/LEHVZK.ec`
61. `le/LEModel.ec` (facade)
62. `sim/Simulator.ec`
63. `games/GameTypes.ec`
64. `games/GameViews.ec`
65. `games/GameAdvantage.ec`
66. `games/GameMSHopTypes.ec`
67. `games/GameMSHopTransitions.ec`
68. `games/GameMSHopComposition.ec`
69. `games/GameMSHops.ec` (facade)
70. `games/GameLEBridge.ec`
71. `games/Games.ec` (facade)
