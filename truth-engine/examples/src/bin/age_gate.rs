...
fn main() {
    ...
    let proof = qssm_local_prover::prove(&setup.ctx, &template, &claim_ok, 100, 50, setup.binding_ctx, entropy_seed)
        .expect("prove should succeed for age 25");
    let ok = qssm_api::verify(&setup.ctx, &template, &claim_ok, &proof, setup.binding_ctx)
    ...
    let err = qssm_local_prover::prove(&setup.ctx, &template, &claim_bad, 100, 50, setup.binding_ctx, entropy_seed);
