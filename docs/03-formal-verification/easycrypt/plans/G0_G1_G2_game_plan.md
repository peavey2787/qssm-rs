...
- Segment game-layer obligations (proof debt; MS1/MS2 now each expose a **single lower `game_pr_ms_core` bridge axiom surface**):
  - **MS1 (hash binding):** canonical bound `A_MS1_canonical_hash_binding_bound` is now a **proved lemma** derived from `A_MS1_hash_binding_step_advantage_bound`, which is itself a **proved lemma** from `A_MS1_hash_binding_concrete_pair_advantage_bound`; that concrete pair theorem is now also a **proved lemma**, obtained by unfolding `Adv` / `game_pr` and applying the lower bridge axiom `A_MS1_hash_binding_game_pr_core_bound` in `GameAdvantage.ec`. **`A_MS1_hash_binding_transition`** remains a **proved lemma** applying the canonical bound directly. Canonical step witness remains available via `L_ms1_hash_binding_step_canonical`. Current blocker: discharging `A_MS1_hash_binding_game_pr_core_bound` from concrete semantics below the abstract `game_pr_ms_core` boundary; `A1_ms_hash_binding_nonneg` / theorem `A1_ms_hash_binding` only give budget nonnegativity.
  - **MS2 (ROM / FS programmability):** canonical bound `A_MS2_canonical_rom_programming_bound` is now a **proved lemma** derived from `A_MS2_rom_programming_step_advantage_bound`, which is itself a **proved lemma** from `A_MS2_rom_programming_concrete_pair_advantage_bound`; that concrete pair theorem is now also a **proved lemma**, obtained by unfolding `Adv` / `game_pr` and applying the lower bridge axiom `A_MS2_rom_programming_game_pr_core_bound` in `GameAdvantage.ec`. **`A_MS2_rom_programming_transition`** remains a **proved lemma** applying the canonical bound directly. Canonical step witness remains available via `L_ms2_rom_programming_step_canonical`. Current blocker: discharging `A_MS2_rom_programming_game_pr_core_bound` from concrete semantics below the abstract `game_pr_ms_core` boundary; `A2_ms_rom_programmability_nonneg` / theorem `A2_ms_rom_programmability` only give budget nonnegativity, and `A2_programmable_oracle_exists` only gives ROM-response existence.
