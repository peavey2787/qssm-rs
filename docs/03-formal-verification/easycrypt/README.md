...
- The LE FS shadow surface now has two checker-validated internal lanes: an exported projected-post lane that still proves the theorem-facing zero-budget bridge lemmas, and a separate shadow-local semantic branch experiment with a genuine two-branch good/bad sampler. On that local semantic lane, the good branch image is exactly the theorem-facing programmed view, the bad branch image is a separate semantic programmed post view, the semantic-post marginal is the branch-choice pushforward over those two images, and its `sdist` from the theorem-facing programmed view is locally bounded by `le_fs_shadow_local_bad_branch_mass = 1%r / 2%r`. `LEFsProgrammingSurface.ec` now also proves `le_fs_shadow_local_bad_branch_mass = epsilon_le_fs_semantic`, where `BudgetParameters.ec` owns `epsilon_le_fs_semantic = 1%r / 2%r`.
- Three parallel semantic LE budget views are now present. The local-mass chain still ends in `epsilon_le_rej + le_fs_shadow_local_bad_branch_mass` and feeds `qssm_main_theorem_semantic_budget`. The owned component-sum chain ends in `epsilon_le_rej + epsilon_le_fs_semantic` and feeds `A_G1_to_G2_le_semantic_owned_budget_transition_bound` plus `qssm_main_theorem_semantic_budget_owned`. The semantic umbrella chain ends in `epsilon_le_semantic` and feeds `A_G1_to_G2_le_semantic_umbrella_transition_bound` plus `qssm_main_theorem_semantic_budget_umbrella`.
...
- The next meaningful refinement is to decide whether `epsilon_le_semantic` should remain a parallel umbrella or become the exported nonzero LE budget path, and then whether the exact-zero and semantic theorem families should keep coexisting permanently or be reorganized around one active semantic route plus an exact-zero corollary family.