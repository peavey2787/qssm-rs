...
- The LE FS shadow surface now has two checker-validated internal lanes: an exported projected-post lane that still proves the theorem-facing zero-budget bridge lemmas, and a separate shadow-local semantic branch experiment with a genuine two-branch good/bad sampler. Both semantic branches have support, the semantic-post marginal is supported exactly on the good and bad branch images, and the semantic failure probability now closes to `1%r / 2%r = le_fs_shadow_local_bad_branch_mass` rather than feeding `epsilon_le_fs`.
...
- The active theorem path is still unchanged: `LEFsProgramming.ec`, `LEStatisticalDistance.ec`, and `MainTheorem.ec` remain on the exact-zero route, and `epsilon_le_fs` / `epsilon_le` still remain `0%r`.
- The next meaningful refinement is to connect the new local FS branch mass to theorem-facing budget arithmetic only after the top-level zero corollary can absorb a nonzero LE FS component safely.