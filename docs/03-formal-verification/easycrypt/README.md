...
Current status: the tree checks cleanly with `./check_easycrypt.sh`; the current checker snapshot is `OK` over 108 explicitly listed theories; `axiom_count=0`; `admit_count=0`; and the active theorem path closes on a concrete exact-zero budget model. That is a machine-checked statement about the current abstraction boundary, not yet a nonzero cryptographic security estimate for the deployed system.
...
- The EasyCrypt tree is checker-green today: `./check_easycrypt.sh` currently reports `OK` over 108 explicitly listed theories.
