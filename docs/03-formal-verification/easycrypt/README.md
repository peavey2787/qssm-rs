...
- The LE FS shadow surface now has two checker-validated internal lanes: an exported projected-post lane that still proves the theorem-facing zero-budget bridge lemmas, and a separate shadow-local semantic branch experiment with a genuine two-branch good/bad sampler. On that local semantic lane, the good branch image is exactly the theorem-facing programmed view, the bad branch image is a separate semantic programmed post view, the semantic-post marginal is the branch-choice pushforward over those two images, and its `sdist` from the theorem-facing programmed view is locally bounded by `le_fs_shadow_local_bad_branch_mass = 1%r / 2%r` rather than feeding `epsilon_le_fs`.
- A parallel theorem-facing semantic LE chain is now present: `A_LE_fs_semantic_programming_sampler_sdist_le_bad_branch_mass`, `A_LE_semantic_combined_hiding_bounds_sdist`, `A_LE_HVZK_semantic_transition_bound`, `A_G1_to_G2_le_semantic_transition_bound`, and `qssm_main_theorem_semantic_budget`. The semantic LE/game bridge ends in `epsilon_le_rej + le_fs_shadow_local_bad_branch_mass`, and the top-level QSSM theorem now uses that bridge directly to prove `epsilon_ms_hash_binding + epsilon_ms_rom_programmability + epsilon_le_rej + le_fs_shadow_local_bad_branch_mass`.
...
- The next meaningful refinement is to move from the current parallel semantic theorem chain to a fully budget-owned nonzero FS component in `BudgetParameters.ec`, then decide whether the exact-zero and semantic bridges should continue to coexist or whether the semantic bridge should become the active exported LE budget path.