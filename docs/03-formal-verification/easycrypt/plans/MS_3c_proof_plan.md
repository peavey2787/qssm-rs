...
| **`d_ms3c_{real,sim}_seed_{challenge,announcement}`**, **`d_ms3c_{real,sim}_payload_seed`**, **`ms3c_{real,sim}_payload_from_seed`** | operator | Real/sim payload seed is a **pair** (challenge material `*` announcement material); joint seed law = **independent product** of the two component laws; **`d_ms3c_{real,sim}_comparison_payload`** = **`dmap`** pushforward (`ComparisonPayloadSeeds.ec`). **`d_ms3c_{real,sim}_seed_challenge`** and **`d_ms3c_real_seed_announcement`** are **`dunit tt`** on **`unit`**; **`d_ms3c_sim_seed_announcement`** and both **`from_seed`** maps remain abstract—see **`ComparisonPayloadSeeds.ec`**. |
...
| **`A_ms3c_sim_seed_announcement_lossless`** | axiom | **`d_ms3c_sim_seed_announcement`** still abstract; should become **`is_lossless`** from a concrete definition (**`duniform_ll`**, ROM products, etc.). |
...
**Goal:** eventually make seed carriers (including **`ms3c_sim_seed_announcement`**, and richer **`ms3c_*_seed_challenge`** / **`ms3c_real_seed_announcement`** than **`unit`**) concrete record or tuple types large enough to hold every **`mscp_*`** field that is **not** fixed deterministically from **`ms_public_input`** alone. Phase-1 **`unit`** + **`dunit`** on real announcement and both challenges is scaffolding only.
...
- **`A_ms3c_*_seed_*_lossless`:** real and sim challenge and real announcement done (**`L_ms3c_{real,sim}_seed_challenge_lossless`**, **`L_ms3c_real_seed_announcement_lossless`**); remaining sim announcement law `by` **`duniform_ll`**, **`dmap_ll`**, **`dprod_ll_auto`** once defined.
...
|-------------|---------------------------|
| **Seed losslessness** | Real and sim **challenge** and real **announcement** laws done (**`dunit`**); one **sim announcement** axiom + standard lossless lemmas remain. |
