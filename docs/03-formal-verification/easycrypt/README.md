...
- **LE:** split under `le/` — `LESurface.ec` (core ops, `epsilon_le`, views, surrogates, game-hop / sdist surface, Set-B and hiding predicates), `LERejectionSampler.ec` (lower execution-facing rejection sampler surface with `d_le_rejection_real_execution_view`, `le_rejection_transform`, and `d_le_rejection_post_execution_view`), `LESetB.ec` (Set-B lemmas + `A_LE_{real,sim}_view_distribution_defined`), `LERejection.ec` (rejection layer + `A_LE_rejection_surrogate_sdist_bound` / half-bound), `LEFsProgramming.ec` (FS layer + `A_LE_fs_surrogate_preserves_shape` / `A_LE_fs_surrogate_sdist_bound`), `LEViewIndist.ec` (view indistinguishability + distribution links), `LEStatisticalDistance.ec` (triangle / distinguisher bridge + `A_LE_view_advantage_bound_from_indistinguishability`), `LEHVZK.ec` (`A_LE_real_sim_transcript_equiv_bound`, `A_LE_SetB_HVZK_bound`, `A_LE_HVZK_transition_bound`); **`le/LEModel.ec`** remains the theorem-facing facade and does not import `LERejectionSampler.ec` yet, keeping the lower sampler boundary separate from the current lemma closure. Theories that use LE **operators** (for example `le_game_hop_adv`, `le_transcript_observable`) also **`require import LESurface`** before `LEModel`, because EasyCrypt does not re-export transitive imports into the client scope. Same named axioms/lemmas and semantics as before the split.
- **LE audit, May 2026:** `le/LEViewIndist.ec`, `le/LEStatisticalDistance.ec`, `le/LEHVZK.ec`, and `games/GameLEBridge.ec` are now lemma-only packaging layers. The live LE axiom surface feeding the theorem is lower: `LESurface.ec` contributes only the budget non-negativity fact `A4_le_hvzk_bound_nonneg`; `LERejection.ec` contributes the rejection-definition / acceptance / output-shape bundle plus `A_LE_rejection_surrogate_sdist_bound`; and `LEFsProgramming.ec` contributes the FS query/oracle/shape bundle plus `A_LE_fs_surrogate_sdist_bound`. The surrogate shape axioms are still declared, but they are currently not referenced on the active `A4_le_hvzk -> A_G1_to_G2_le_transition_bound -> MainTheorem` path. The highest-leverage first LE target is therefore `A_LE_rejection_surrogate_sdist_bound`, because it sits on the unique quantitative branch into `A_LE_view_advantage_bound_from_indistinguishability` and also fixes the post-rejection surface that the FS half-hop is built over.
...
- **LE rejection lower target, May 2026:** `le/LERejection.ec` now records the intended lower theorem surface under the name `A_LE_rejection_sampler_sdist_bound` as a comment-only skeleton. That target is not yet provable at the current abstraction boundary: `d_le_real_view` is still an abstract operator in `LESurface.ec`, and `d_le_post_rejection_view` is only the definitional `dmap` push-forward of `d_le_real_view` through abstract `le_post_rejection_surrogate`. The missing next implementation step is therefore a concrete LE rejection execution / sampler distribution below `LESurface.ec`, not another wrapper theorem at the current facade.
- **LE rejection sampler surface, May 2026:** `le/LERejectionSampler.ec` now introduces that lower boundary explicitly. It adds abstract lower ops `d_le_rejection_real_execution_view` and `le_rejection_transform`, plus the concrete push-forward `d_le_rejection_post_execution_view = dmap ... le_rejection_transform`, and records comment-only bridge targets `le_real_view_matches_rejection_execution`, `le_post_rejection_view_matches_execution_transform`, and `A_LE_rejection_sampler_sdist_bound`. Import direction stays clean: the new file depends only on `LESurface.ec` and is not yet imported back into `LERejection.ec` or `LEModel.ec`.
