...
- `qssm_main_theorem_realworld_concrete_256` exists and is closed as the concrete external-bound real-world theorem over four explicit component-bound premises.
- `qssm_main_theorem_realworld_concrete_256_5_over_2_226` exists and is closed as the concrete external-bound `5 / 2^226` companion theorem over the same four explicit premises.
- `qssm_main_theorem_realworld_concrete_256_with_all_reductions` exists and is closed as the fully reduction-facing concrete real-world sibling theorem over four explicit reduction obligations.
- `qssm_main_theorem_realworld_concrete_256_with_all_reductions_5_over_2_226` exists and is closed as the fully reduction-facing `5 / 2^226` companion theorem over the same four explicit reduction obligations.
...
| `qssm_main_theorem_realworld_concrete_256` | `RealWorldBudgetInstantiation.ec` | Closed | Concrete external-bound theorem over `realworld_budget_concrete_256` with four explicit component-bound premises |
| `qssm_main_theorem_realworld_concrete_256_5_over_2_226` | `RealWorldBudgetInstantiation.ec` | Closed | Concrete external-bound companion theorem exposing the closed form `5 / 2^226` under the same four explicit component-bound premises |
| `qssm_main_theorem_realworld_concrete_256_with_all_reductions` | `RealWorldBudgetInstantiation.ec` | Closed | Fully reduction-facing concrete external-bound sibling theorem over four explicit reduction obligations |
| `qssm_main_theorem_realworld_concrete_256_with_all_reductions_5_over_2_226` | `RealWorldBudgetInstantiation.ec` | Closed | Fully reduction-facing concrete companion theorem exposing `5 / 2^226` under the same four explicit reduction obligations |

## Route Split

The current theorem system has seven distinct routes.
...
| Concrete external-bound real-world route | `qssm_main_theorem_realworld_concrete_256`, `qssm_main_theorem_realworld_concrete_256_5_over_2_226`, `qssm_main_theorem_realworld_concrete_256_with_all_reductions`, `qssm_main_theorem_realworld_concrete_256_with_all_reductions_5_over_2_226` | Live | Instantiates the real-world budget record at component epsilon `1 / 2^226` and top epsilon `5 / 2^226`; the original pair keeps explicit component-bound premises, while the newest sibling pair keeps the duplicate MS2 charge explicit and consumes four explicit reduction obligations |
...
| Concrete instantiation layer | `RealWorldBudgetInstantiation.ec` | `realworld_budget_concrete_256`, `qssm_realworld_obligations_concrete_256_from_component_bounds`, the original concrete theorem pair, and additive sibling theorems ending at `qssm_main_theorem_realworld_concrete_256_with_all_reductions` |
...
That semantic distinction and its charged closure are reflected in this route map and in [SPEC_FORMAL_CONFORMANCE_AUDIT.md](SPEC_FORMAL_CONFORMANCE_AUDIT.md). No remaining localized replay seams are expected on the current uniform finite-support / contiguous-layout profile family.