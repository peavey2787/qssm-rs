...
async fn publish_heartbeat(
    ...
    let hb = match collect_local_heartbeat() {
        Ok(h) => h,
        Err(e) => {
            tracing::debug!(target: "p2p_net", "heartbeat skipped: {e}");
            return Ok(false);
        }
    };
...
pub(crate) fn refresh_immune_snapshot_locked(
    ...
    snapshot.governor_state = match governor.governor_state() {
        governor::GovernorState::Expanding => "Expanding".to_string(),
        governor::GovernorState::Defending => "Defending".to_string(),
    };
}
