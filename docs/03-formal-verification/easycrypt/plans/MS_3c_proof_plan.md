...
**MS-3b handoff status:** the phase-1 concrete comparison carrier now exists. **`ms3b_phase1_comparison_carrier`** in **`ms/true_clause/TrueClauseTypes.ec`** fixes canonical operand bits **`[true]`** / **`[false]`**, true-clause index **`0`**, and a concrete Schnorr opening pair **`(sch_pubkey witness, witness)`** for the true clause; **`ms3c_phase1_payload_from_public_input`** in **`ms/comparison/ComparisonPayloadFromSeed.ec`** now consumes the same true-clause index / announcement / share via **`ms3c_phase1_comparison_carrier_from_public_input`**. With that phase-1 carrier in place, **`A_ms3b_operand_hdb_implies_value_gt_target`** is now a **proved lemma** in **`ms/true_clause/TrueClauseTheorem.ec`**, so there are no remaining MS-3b-specific axioms. The next MS-3c work is therefore no longer “remove the last MS-3b leaf”; it is to refine this concrete phase-1 carrier from canonical placeholder data into transcript- or execution-backed comparison material.
...
| Risk | Mitigation |
|------|------------|
| **MS-3b carrier is concrete but still Phase-1 canonical** | Replace **`ms3b_phase1_comparison_carrier`**’s fixed operands **`[true]`** / **`[false]`** and placeholder true-clause opening with transcript- or execution-backed comparison material; keep **`ComparisonPayloadFromSeed.ec`** consuming the same carrier so MS-3b and MS-3c stay aligned. |
...
| **Game view only carries the public observable slice today** | If richer comparison payloads need transcript-backed announcement/share fields, extend the game-view observable spine beyond **`ms_game_view_public_obs xms`** so comparison sampling can depend on frozen transcript material instead of phase-1 placeholders. |
...
See **§ Concrete MS-3c payload constructor design** above: prefer **richer seed types + `from_seed` + component laws**, keeping **`stmt := ms3c_public_stmt_digest x`** for **`mscp_query_digest`** (digest constructor is **proved** for Phase-1; no `forall stmt`). The next concrete target is to replace the current phase-1 MS-3b comparison carrier with transcript- or execution-backed operand bits and a non-placeholder true-clause opening while preserving the shared carrier link into **`ms3c_phase1_payload_from_public_input`**. In parallel: thread richer comparison material through **`ms_transcript_observable`** / **`games/GameViews.ec`** as needed, and replace placeholder statement or announcement fields in the MS-3c public surface with transcript-backed data. **Game hop:** no additional MS-3c bridge axiom remains; **`A_MS3c_comparison_bundle_implies_game_pr_equality`** and **`A_MS3c_canonical_comparison_exact_bound`** are both proved lemmas.
