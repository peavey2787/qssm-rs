...
Alongside that exact-zero path, the tree now also carries a checked semantic-budget theorem path for semantic LE modeling, and that semantic theorem path now retargets only its MS term through the parallel primitive MS ROM semantic owner skeleton while the exact-zero public route remains unchanged. The intended public citation targets are `qssm_main_theorem` for the exact-zero abstraction theorem and `qssm_main_theorem_semantic_budget` for the semantic-budget theorem; `qssm_main_theorem_nonzero_budget` is a façade alias for discoverability.

The semantic MS owner is now also grounded by a comparison-local execution-owned bridge theory. `ms/comparison/ComparisonPayloadSemanticBridge.ec` defines `ms_rom_semantic_state_of_category_execution_seed`, the category interpretation predicates and lemmas, `ms_rom_local_failure_mass`, the semantic AfterRom projection `ms_rom_semantic_after_rom_observable_of_state`, and `A_MS2_rom_programming_execution_owned_semantic_bound`; `ms/MSProbabilitySurface.ec` now maps that projection over `d_ms_rom_semantic_coupled_state` as `d_ms_after_rom_semantic_observable_v2`, proves `L_ms2_rom_programming_transition_le_execution_owned_semantic_failure`, and keeps `A_MS2_rom_programming_semantic_transition_bound` on the same theorem name while `games/GameMSHopComposition.ec` and `theorem/MainTheorem.ec` remain untouched in this step.

Beside that routed semantic MS2 surface, `BudgetParameters.ec` now also carries a parallel semantic MS1 hash-binding owner skeleton with categories `clean`, `collision`, `malformed_binding`, and `transcript_mismatch`, demo counts `13,1,1,1`, and owner term `epsilon_ms_hash_binding_semantic = 3%r / 16%r`. `primitives/FS.ec` re-exports it as `epsilon_ms_hash_binding_semantic`, `ms/MS.ec` now stages the MS-side alias/nonneg surface, and `ms/source/SourceHashBindingSemanticBridge.ec` now gives it a source-local execution-owned bridge plus a staged lower/game sibling chain through `A_G0_to_G1_ms_hash_binding_semantic_transition_bound`. The live semantic theorem route now also consumes semantic MS1 through `A_G0_to_G1_ms_semantic_transition_bound`, while the exact-zero public route still consumes `epsilon_ms_hash_binding = 0%r`.
...
- `theorem/MainTheorem.ec` now exposes two public theorem modes: `qssm_main_theorem` is the exact-zero abstraction theorem, and `qssm_main_theorem_semantic_budget` is the preferred semantic-budget theorem closing at `epsilon_ms_hash_binding_semantic + epsilon_ms_rom_programmability_semantic + epsilon_le_semantic`
...
- `ms/source/SourceHashBindingSemanticBridge.ec` now gives that owner a source-local execution-owned bridge via `ms_hash_binding_semantic_state_of_category_source`, `ms_hash_binding_semantic_category_condition`, `ms_hash_binding_local_failure_mass`, and `A_MS1_hash_binding_execution_owned_semantic_bound`
...
- `games/GameMSHopComposition.ec` now also carries the staged sibling `A_G0_to_G1_ms_hash_binding_semantic_transition_bound`, while the existing live semantic theorem `A_G0_to_G1_ms_semantic_transition_bound` and `theorem/MainTheorem.ec` still consume `epsilon_ms_hash_binding = 0%r`
...
- `ms/comparison/ComparisonPayloadSemanticBridge.ec` now gives that MS ROM owner a comparison-local execution-owned bridge surface via `ms_rom_semantic_state_of_category_execution_seed`, `ms_rom_semantic_category_condition`, `ms_rom_local_failure_mass`, and `A_MS2_rom_programming_execution_owned_semantic_bound`
- `ms/MSProbabilitySurface.ec` now also carries the semantic endpoint `d_ms_after_rom_semantic_observable_v2` and the local bound `L_ms2_rom_programming_transition_le_execution_owned_semantic_failure`, while `A_MS2_rom_programming_semantic_transition_bound` still depends directly on `A_MS2_rom_programming_execution_owned_semantic_bound` and the exact-zero `A_MS2_rom_programming_transition_bound` remains unchanged
...
- retargeting the public MS2 AfterRom stage away from the current exact-equality carrier now that the comparison-local execution-owned semantic endpoint exists below it
