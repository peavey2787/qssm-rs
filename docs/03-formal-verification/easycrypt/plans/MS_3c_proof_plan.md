...
- **Payload-visible sampled coins:** **`ms3c_comparison_clause_payload`** now carries payload-only **`mscp_rom_coin`** and **`mscp_transcript_coin`** fields. **`ComparisonPayloadFromSeed.ec`** threads the sampled seed coins into those fields, while **`ms3c_make_clause_surface`** ignores them so the coupling and game layers remain surface-oriented.
...
- **Support-equality relaxation:** the old whole-payload support equalities in **`ComparisonPayloadFromSeed.ec`** / **`ComparisonPayloadSupportTypes.ec`** are now folded-surface equalities only. The new lemmas **`L_ms3c_{real,sim}_payload_from_seed_visible_coin_fields`** expose the sampled-coin propagation separately.
...
No new hook or axiom was added for the support-equality relaxation; the MS-3c assumption surface remains the six rows above.
...
**Still open:** Phase-1 comparison data and seed carriers are native, the component laws sample ROM/transcript scalar coins, the coupling layer no longer depends on collapsing paired payloads to the same support point, and those sampled coins are now visible in payload-only fields. The next MS-3c work is to let those visible coin fields drive richer transcript/ROM-execution structure without changing the compared public/share surface or reopening the coupling shell. The **`A_ms3c_{real,sim}_from_seed_uses_*`** facts are now **support-local proved lemmas** in **`ComparisonPayloadSeedAnchors.ec`**, all four component samplers remain lossless, and the remaining proof debt is therefore in non-degenerate payload / execution modeling, not in placeholder or unit comparison carriers.
...
**Remaining design boundary:** the carrier plumbing is now native, the seed samplers now genuinely sample ROM/transcript coins, and those sampled coins are visible in payload-only fields while the public game boundary remains closed because the folded comparison surface is still the native Phase-1 image on support. Future work is to keep that boundary fixed while allowing those visible coin fields to drive richer transcript or ROM state.
