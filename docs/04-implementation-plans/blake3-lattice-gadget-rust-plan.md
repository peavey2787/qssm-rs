...
| **6** | **`io/prover_json.rs`** + **`examples/l2_handshake.rs`** + deployment docs: **`SovereignWitness` / Merkle** → JSON via **`io::prover_json`** helpers, **`prover_package.json`**, R1CS manifest path, wire-count metadata. | Example runs; artifacts on disk; manifest line count **65 184**; **`docs/l2-kaspa-core-deployment-manifest.md`** aligned. |
...
| [`crates/qssm-gadget/src/lib.rs`](crates/qssm-gadget/src/lib.rs) | Declares **`primitives`**, **`lattice`**, **`circuit`**, **`io`**, **`merkle`**, **`error`**; thin **`binding` / `bits` / …** modules re-export those layers so **`qssm_gadget::binding::…`** paths stay stable; crate‑root **`pub use`** for main types. |
...
| [`crates/qssm-gadget/src/primitives/blake3_compress.rs`](crates/qssm-gadget/src/primitives/blake3_compress.rs) | **Phase 5**: **`MSG_SCHEDULE`**, **`CompressionWitness`** (**56 × `G`**), **`hash_merkle_parent_witness`**, **`compress_native`** oracle (JSON helpers live in **`io/prover_json.rs`**). |
...
| [`crates/qssm-gadget/src/circuit/template.rs`](crates/qssm-gadget/src/circuit/template.rs) | **`QssmTemplate`**, anchor kinds, **`lattice::predicate`** integration for claim checks. |
...
| [`crates/qssm-gadget/src/lattice/predicate.rs`](crates/qssm-gadget/src/lattice/predicate.rs) | Declarative **`PredicateBlock`** / **`CmpOp`** for template verification. |
| [`crates/qssm-gadget/src/io/prover_json.rs`](crates/qssm-gadget/src/io/prover_json.rs) | Phase 6 JSON: private wire counting; **`compression_witness_to_prover_json`** / **`merkle_parent_hash_witness_to_prover_json`** (Merkle / compress witnesses no longer carry JSON methods on the primitive structs). |
