...
- **LE:** `le/LEModel.ec` — non-vacuous `set_b_parameter_well_formed` predicate, `A4_le_hvzk_bound_nonneg`, LE hop carrier `le_game_hop_adv`, transcript predicate `le_real_sim_transcript_equiv`, and layered LE-HVZK interface predicates `le_set_b_params_ok`, `le_rejection_sampling_bound_ok`, `le_fs_programming_bound_ok`, `le_hvzk_bound`; narrower LE obligations `A_LE_SetB_params_sound`, `A_LE_rejection_sampling_hiding_bound`, `A_LE_fs_programming_bound`, `A_LE_real_sim_transcript_equiv_bound`; derived lemma `A_LE_SetB_HVZK_bound`; wrapper lemma `A_LE_HVZK_transition_bound`
