...
1. **MS-3a (residual)** — discharge **`A_ms3a_payload_dmap_bitness_layer_schedule`** and the three **seed-support** axioms (**`A_ms3a_real_seed_programmed_on_support`**, **`A_ms3a_sim_seed_programmed_on_support`**, **`A_ms3a_seed_pair_public_fields_on_support`**) by fixing abstract **`d_ms3a_*_payload_seed`** / **`ms3a_*_payload_from_seed`** from execution/games; payload-layer **`ms3a_payload_*`** lemmas are already **proved** from seeds; see `plans/MS_3a_proof_plan.md`.
