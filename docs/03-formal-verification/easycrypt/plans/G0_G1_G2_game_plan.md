...
- Segment game-layer obligations (proof debt; MS1/MS2 now each expose a **single explicit stage-pair axiom surface**):
  - **MS1 (hash binding):** canonical bound `A_MS1_canonical_hash_binding_bound` is now a **proved lemma** derived from `A_MS1_hash_binding_step_advantage_bound`, which is itself a **proved lemma** from the explicit stage-pair axiom `A_MS1_hash_binding_concrete_pair_advantage_bound`. **`A_MS1_hash_binding_transition`** remains a **proved lemma** applying the canonical bound directly. Canonical step witness remains available via `L_ms1_hash_binding_step_canonical`. Current blocker: the repo still has no lower theorem proving `game_pr_ms_core ... MSGameStageReal ... - game_pr_ms_core ... MSGameStageAfterBinding ... <= epsilon_ms_hash_binding`; `A1_ms_hash_binding_nonneg` / theorem `A1_ms_hash_binding` only give budget nonnegativity.
  - **MS2 (ROM / FS programmability):** canonical bound `A_MS2_canonical_rom_programming_bound` is now a **proved lemma** derived from `A_MS2_rom_programming_step_advantage_bound`, which is itself a **proved lemma** from the explicit stage-pair axiom `A_MS2_rom_programming_concrete_pair_advantage_bound`. **`A_MS2_rom_programming_transition`** remains a **proved lemma** applying the canonical bound directly. Canonical step witness remains available via `L_ms2_rom_programming_step_canonical`. Current blocker: the repo still has no lower theorem proving `game_pr_ms_core ... MSGameStageAfterBinding ... - game_pr_ms_core ... MSGameStageAfterRom ... <= epsilon_ms_rom_programmability`; `A2_ms_rom_programmability_nonneg` / theorem `A2_ms_rom_programmability` only give budget nonnegativity, and `A2_programmable_oracle_exists` only gives ROM-response existence.
  ...
  - **MS3c (comparison exact simulation):** **`ms/comparison/`** is **axiom-free**; schedule-level **`ms_comparison_exact_simulation_equiv`** is packaged by **`MS_3c_exact_comparison_simulation`** in **`ms/MS.ec`**. The **game** gap is axiom **`A_MS3c_comparison_bundle_implies_game_pr_equality`** in **`games/GameMSHopTypes.ec`** (the MS-3c implication bundle ⇒ equality of abstract **`game_pr`** on **`G_MS_after_comparison`** vs **`G_MS_sim`**). Lemma **`A_MS3c_canonical_comparison_exact_bound`** (same **`Adv … <= 0%r`** conclusion as before) is **proved** from **`Adv_def`** + that axiom. **`A_MS3c_comparison_transition`** remains a **proved lemma** applying the canonical bound. Canonical step witness remains via `L_ms3c_comparison_exact_step_canonical`.
