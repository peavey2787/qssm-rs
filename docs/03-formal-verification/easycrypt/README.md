...
- **LE FS lower surface, May 2026:** `le/LEFsProgrammingSurface.ec` now proves both lower bridge facts `le_fs_surrogate_matches_programmed_view` and `le_fs_programming_preserves_shape_lower`. `le/LEFsProgramming.ec` imports that lower surface and replaces `A_LE_fs_surrogate_preserves_shape` with a lemma. The remaining FS work is therefore quantitative: `A_LE_fs_surrogate_sdist_bound` still needs an honest lower theorem such as `A_LE_fs_programming_sampler_sdist_bound`.
