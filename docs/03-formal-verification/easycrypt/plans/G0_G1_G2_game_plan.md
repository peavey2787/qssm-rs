...
- Segment game-layer obligations (proof debt; MS1 is split into a **narrow axiom** plus a **lemma**):
  - **MS1 (hash binding):** primary obligation is now canonical axiom `A_MS1_canonical_hash_binding_bound` (**axiom**) on `Adv (G_MS_real x xms s) (G_MS_after_binding x xms s) D <= epsilon_ms_hash_binding`; **`A_MS1_hash_binding_transition`** is a **proved lemma** that applies this canonical bound directly. The older generic step bound `A_MS1_hash_binding_replacement_bound` remains as a compatibility wrapper for `ms1_hash_binding_step src dst xms` APIs and is no longer the main proof target. Canonical step witness remains available via `L_ms1_hash_binding_step_canonical`. Cryptographic discharge (e.g. Blake3) stays out of this skeleton; budget ties to `ms/MS.ec` (`epsilon_ms_hash_binding`, `A1_ms_hash_binding_nonneg`) and theorem `A1_ms_hash_binding`.
  - **MS2 (ROM / FS programmability):** primary obligation is now canonical axiom `A_MS2_canonical_rom_programming_bound` (**axiom**) on `Adv (G_MS_after_binding x xms s) (G_MS_after_rom x xms s) D <= epsilon_ms_rom_programmability`; **`A_MS2_rom_programming_transition`** is a **proved lemma** that applies this canonical bound directly. The older generic step bound `A_MS2_rom_programming_replacement_bound` remains as a compatibility wrapper for `ms2_rom_programming_step src dst xms` APIs and is no longer the main proof target. Canonical step witness remains available via `L_ms2_rom_programming_step_canonical`. Budget and FS interface: `primitives/FS.ec` (`epsilon_ms_rom_programmability`, `A2_ms_rom_programmability_nonneg`, `A2_programmable_oracle_exists`); theorem `A2_ms_rom_programmability`.
  - **MS3a (bitness exact simulation):** primary obligation is now canonical axiom `A_MS3a_canonical_bitness_exact_bound` (**axiom**) on `Adv (G_MS_after_rom x xms s) (G_MS_after_bitness x xms s) D <= 0%r` under `ms3a_bitness_real_sim_equiv xms s`; **`A_MS3a_bitness_transition`** is a **proved lemma** that applies this canonical bound directly. The older generic step bound `A_MS3a_bitness_exact_step_bound` remains as a compatibility wrapper for `ms3a_bitness_exact_step src dst xms s` and is no longer the main proof target. Canonical step witness remains available via `L_ms3a_bitness_exact_step_canonical`. Source proof debt stays in **`ms/source/`** (`MS_3a_exact_bitness_simulation` / `SourceTheorem`).
  - **MS3b (true clause):** primary obligation is now canonical axiom `A_MS3b_canonical_true_clause_bound` (**axiom**) on `Adv (G_MS_after_bitness x xms s) (G_MS_after_comparison x xms s) D <= 0%r` under the existing MS-3b forall bundle; **`A_MS3b_true_clause_transition`** is a **proved lemma** that applies this canonical bound directly. The older generic step bound `A_MS3b_true_clause_exact_step_bound` remains as a compatibility wrapper for `ms3b_true_clause_exact_step src dst xms` and is no longer the main proof target. Canonical step witness remains available via `L_ms3b_true_clause_exact_step_canonical`. Proof debt in **`ms/TrueClause.ec`** (`MS_3b_true_clause_characterization` and hooks).
  - **MS3c (comparison exact simulation):** primary obligation is now canonical axiom `A_MS3c_canonical_comparison_exact_bound` (**axiom**) on `Adv (G_MS_after_comparison x xms s) (G_MS_sim x xms s) D <= 0%r` under the existing MS-3c implication bundle; **`A_MS3c_comparison_transition`** is a **proved lemma** that applies this canonical bound directly. The older generic step bound `A_MS3c_comparison_exact_step_bound` remains as a compatibility wrapper for `ms3c_comparison_exact_step src dst xms s` and is no longer the main proof target. Canonical step witness remains available via `L_ms3c_comparison_exact_step_canonical`. Proof debt in **`ms/Comparison.ec`** (`MS_3c_exact_comparison_simulation` and payload/scheduling obligations).
