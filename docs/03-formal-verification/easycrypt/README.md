...
- **LE:** split under `le/` — `LESurface.ec` (core ops, `epsilon_le`, views, surrogates, game-hop / sdist surface, Set-B and hiding predicates; `le_post_rejection_surrogate` is now the identity), `LERejectionSampler.ec` (lower rejection-sampler bridge surface with alias ops `d_le_rejection_real_execution_view`, `le_rejection_transform`, `d_le_rejection_post_execution_view`, plus proved bridge lemmas `le_real_view_matches_rejection_execution` and `le_post_rejection_view_matches_execution_transform`), `LEFsProgrammingSurface.ec` (lower FS-programming surface with concrete query/programmed-response carriers `le_fs_query_row` and `le_fs_programmed_response_carrier`, plus `d_le_pre_fs_programming_view`, `le_fs_surrogate_transform`, and `d_le_post_fs_programmed_view`), `LESetB.ec` (Set-B lemmas + `A_LE_{real,sim}_view_distribution_defined`), `LERejection.ec` (rejection layer + proved `A_LE_rejection_surrogate_preserves_shape`, proved zero-distance `A_LE_rejection_surrogate_sdist_bound`, proved `A_LE_rejection_sampler_sdist_bound`, and half-bound packaging), `LEFsProgramming.ec` (FS layer + `A_LE_fs_surrogate_preserves_shape` / `A_LE_fs_surrogate_sdist_bound`), `LEViewIndist.ec` (view indistinguishability + distribution links), `LEStatisticalDistance.ec` (triangle / distinguisher bridge + `A_LE_view_advantage_bound_from_indistinguishability`), `LEHVZK.ec` (`A_LE_real_sim_transcript_equiv_bound`, `A_LE_SetB_HVZK_bound`, `A_LE_HVZK_transition_bound`); **`le/LEModel.ec`** remains the theorem-facing facade and does not import either lower surface file, while **`le/LERejection.ec`** imports the rejection sampler bridge layer and **`le/LEFsProgramming.ec`** still remains above the new FS surface. Theories that use LE **operators** (for example `le_game_hop_adv`, `le_transcript_observable`) also **`require import LESurface`** before `LEModel`, because EasyCrypt does not re-export transitive imports into the client scope. Same named axioms/lemmas and semantics as before the split.
- **LE audit, May 2026:** `le/LEViewIndist.ec`, `le/LEStatisticalDistance.ec`, `le/LEHVZK.ec`, and `games/GameLEBridge.ec` are lemma-only packaging layers. The live LE axiom surface feeding the theorem is now lower than the rejection surrogate itself: `LESurface.ec` contributes only the budget non-negativity fact `A4_le_hvzk_bound_nonneg`; `LERejection.ec` contributes the rejection-definition / acceptance / output-shape bundle, but its surrogate-preservation and surrogate-sdist obligations are now lemmas; and `LEFsProgramming.ec` contributes the FS query/oracle/shape bundle plus `A_LE_fs_surrogate_preserves_shape` and `A_LE_fs_surrogate_sdist_bound`. On the active `A4_le_hvzk -> A_G1_to_G2_le_transition_bound -> MainTheorem` path, the remaining quantitative LE axiom is now the FS half-hop, while the remaining real-side abstraction blocker is still `d_le_real_view`.
...
- **LE FS audit, May 2026:** the same cleanup is not yet honest on the FS side. At the current abstraction boundary, `le/LESurface.ec` exposes `le_fs_view_surrogate : le_transcript_observable -> le_transcript_observable` over an abstract carrier with only accessor ops and no constructor surface below it, and there is no lower FS sampler/execution layer analogous to `LERejectionSampler.ec`. The only local concrete definition available in that setting is effectively the identity map; using it would collapse `A_LE_fs_surrogate_sdist_bound` to zero by fiat and erase the remaining FS-programming assumption rather than discharge it from a concrete FS analysis. The next honest FS step is therefore a lower FS surface or a concrete LE observable constructor below `LESurface.ec`, not an identity instantiation of `le_fs_view_surrogate`.
- **LE FS lower surface, May 2026:** `le/LEFsProgrammingSurface.ec` now supplies that lower boundary without changing the FS proof obligations yet. It introduces concrete lower names for the FS query row and programmed-response carrier, defines `d_le_pre_fs_programming_view` and `d_le_post_fs_programmed_view`, aliases the lower `le_fs_surrogate_transform` to the current facade operator, and proves the first bridge fact `le_fs_surrogate_matches_programmed_view` definitionally. The next lower shape theorem `le_fs_programming_preserves_shape_lower` is still blocked exactly because `le_fs_surrogate_transform` only unfolds to the abstract facade operator `le_fs_view_surrogate`: below `LEFsProgramming.ec` there is still no concrete constructor for `le_transcript_observable` and no fieldwise equality theorem for that transform. Import direction stays clean: the new file depends only on `LESurface.ec`, `LEFsProgramming.ec` does not import it yet, and no new axioms were added.
- **LE rejection sampler surface, May 2026:** `le/LERejectionSampler.ec` continues to bridge the lower sampler names back to the current LE facade definitionally. `d_le_rejection_real_execution_view` aliases `d_le_real_view`, `le_rejection_transform` aliases the now-concrete identity rejection surrogate, and `d_le_rejection_post_execution_view` remains the push-forward `dmap ... le_rejection_transform`. Import direction stays clean enough for the current phase: the sampler file still depends only on `LESurface.ec`, `LERejection.ec` imports it, and `LEModel.ec` still does not depend on it. The remaining lower real-side blocker is `d_le_real_view`, not the rejection transform.
