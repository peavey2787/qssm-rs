...
- **Checker:** **`./check_easycrypt.sh`** passes across all 78 EasyCrypt theories.
- **Axiom count:** 32 total **`axiom`** declarations under **`docs/03-formal-verification/easycrypt`**.
...
- **MS-3c closure / handoff:** the comparison hop is now closed at the game boundary. No further MS-3c game-layer axiom is needed, and the remaining axiom surface has moved out of the MS game lane entirely.
- **Payload-visible sampled coins:** **`ms3c_comparison_clause_payload`** now carries payload-only **`mscp_rom_coin`** and **`mscp_transcript_coin`** fields. **`ComparisonPayloadFromSeed.ec`** threads the sampled seed coins into those fields, then derives a coin-driven ROM row **`(mscp_query_digest, mscp_rom_coin)`** and a coin-driven comparison-opening transcript view from the same payload. Decision for this phase: those auxiliary views stay proof-local, and **`ms3c_make_clause_surface`** still ignores them so the coupling and game layers remain surface-oriented.
- **Comparison execution-seed package:** **`ComparisonPayloadSeedTypes.ec`** now defines **`ms3c_execution_seed_package`** plus real/sim execution-seed laws **`d_ms3c_{real,sim}_execution_seed`** that store the original challenge/announcement seeds together with the auxiliary ROM row and non-public comparison-opening bundle. **`ComparisonPayloadFromSeed.ec`** adds **`d_ms3c_{real,sim}_execution_comparison_payload`** and proves those execution-facing payload laws are definitionally equal to the existing comparison payload laws after projection. Decision: keep this package comparison-local for now, rather than threading it through the broader source/game boundary.
...
Decision for this phase: keep the auxiliary coin-driven ROM/transcript views inside the comparison lane, and keep the resulting execution-seed package comparison-local. They now feed the comparison-side execution-seed package in **`ComparisonPayloadSeedTypes.ec`** / **`ComparisonPayloadFromSeed.ec`**, but **`SourceModel.ec`** currently stops at native public comparison openings and no source/game theorem consumes a non-public comparison execution carrier. Threading the package upward would therefore require new non-public interfaces and link lemmas without strengthening any current game proof, so broader source/game integration is deferred.

Implemented concrete non-public execution consumer: a comparison-side execution-seed packaging boundary modeled on **`ms/source/SourceRealExecutionSeed.ec`**. That boundary is now live: **`ms3c_seed_challenge_coin_driven_rom_row`** / **`ms3c_payload_coin_driven_rom_row`** feed the internal ROM query/response row of **`d_ms3c_{real,sim}_execution_seed`**, and **`ms3c_seed_announcement_coin_driven_transcript_openings`** / **`ms3c_payload_coin_driven_transcript_openings`** feed the non-public comparison-opening bundle carried by that execution seed package before the public surface is folded by **`ms3c_make_clause_surface`**.
...
| **`d_ms3c_{real,sim}_execution_seed`** | **definition** (`ComparisonPayloadSeedTypes.ec`) | Execution-facing comparison seed law: a **`dmap`** of the existing real/sim payload-seed law into **`ms3c_execution_seed_package`**, carrying the original seed components plus the auxiliary ROM row and transcript-opening bundle. |
| **`L_ms3c_{real,sim}_execution_seed_support_inv`** | **lemma** (`ComparisonPayloadSeedTypes.ec`) | Support inversion for the execution-seed law: every execution seed package on support comes from an underlying real/sim payload seed. |
| **`L_ms3c_{real,sim}_execution_payload_aux_views_match`** | **lemma** (`ComparisonPayloadFromSeed.ec`) | The execution-seed package's internal ROM row and non-public comparison-opening bundle agree exactly with the payload-level auxiliary views after projecting back through **`from_seed`**. |
| **`A_ms3c_{real,sim}_comparison_payload_matches_execution_seed_law`** | **lemma** (`ComparisonPayloadFromSeed.ec`) | The comparison payload laws are unchanged after introducing the execution-seed package: the execution-facing payload law is definitionally the old payload law after projection from **`ms3c_execution_seed_package`**. |
...
MS-3c is closed for the current game layer. The next phase is **MS-3d**: keep the comparison-side execution-seed package comparison-local, treat **`L_ms3c_game_view_public_obs_aligns_v2`**, **`L_ms_MS3c_public_obs_matches_phase1_seed`**, **`L_ms_MS3c_public_obs_matches_phase1_payload`**, **`A_MS3c_comparison_bundle_implies_game_pr_equality`**, and **`A_MS3c_canonical_comparison_exact_bound`** as stable handoff lemmas, and move the remaining MS game-layer proof debt to the narrow MS1/MS2 obligations in **`games/GameMSHopTypes.ec`** plus the abstract probability interface in **`games/GameAdvantage.ec`**.

See **`plans/MS_3d_proof_plan.md`** for the next-phase blocker table and proof order.

The current phase has replaced raw placeholders and digest-backed comparison openings with a native comparison slice on **`ms_public_input`**, native comparison openings on **`ms_transcript_observable`**, and explicit game-layer alignment lemmas for the structured Phase-1 seed/payload surface. The comparison-side execution-seed package modeled on **`SourceRealExecutionSeed.ec`** is now implemented and is the active non-public consumer for the auxiliary ROM row and transcript-opening bundle. The boundary decision is to keep that package comparison-local until a future source/game execution theorem needs a non-public comparison carrier above **`SourceModel.ec`**. **Game hop:** no additional MS-3c bridge axiom remains; **`A_MS3c_comparison_bundle_implies_game_pr_equality`** and **`A_MS3c_canonical_comparison_exact_bound`** are both proved lemmas.
