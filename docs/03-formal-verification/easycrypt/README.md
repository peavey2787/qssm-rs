...
Current status: the tree checks cleanly with `./check_easycrypt.sh`; the current checker snapshot is `OK` over 129 checked theories; `axiom_count=0`; `admit_count=0`; the exact-zero route is unchanged; and the live demo semantic route still closes at `qssm_main_theorem_semantic_budget = 3%r / 4%r`. That is a machine-checked statement about the current abstraction boundary, not yet a production-count cryptographic security estimate for the deployed system.
...
- The parameterized LE route now carries a live rejection-only sampler companion below the theorem-facing wrappers: `LERejectionSamplerParameterizedCore.ec` owns the parameterized rejection branch/marginal core, `LERejectionSamplerMassLiveParameterized.ec` owns the live parameterized rejection mass and `sdist` facts, `LEFsProgrammingParameterizedView.ec` owns the FS-facing parameterized programmed-view/midpoint bridge, `LERejectionParameterized.ec` retargets theorem-facing rejection bounds to that sampler lane, and `LEStatisticalDistanceParameterized.ec` now composes over the parameterized rejection midpoint. The active parameterized rejection profile is `soft=1`, `hard=1`, `invalid=1`, `accept=29`, `failure=3`, `total=32`, so `epsilon_le_rej_parameterized = 3%r / 32%r`; the demo semantic route and exact-zero route remain unchanged, and only LE rejection has been honestly lowered below `3%r / 16%r` so far.
...
- The EasyCrypt tree is checker-green today: `./check_easycrypt.sh` currently reports `OK` over 129 checked theories.
...
- The active parameterized LE rejection route now reaches `qssm_main_theorem_parameterized_budget` through the new live sampler / FS-facing midpoint chain, while LE FS still remains on the existing demo-derived bad-branch bridge and no MS parameter family has been honestly lowered yet.
