...
This section is a **design-only** blueprint to discharge the remaining **`A_ms3c_*`** axioms in **`ms/comparison/`** (digest constructor wiring, coupling fragments, false-clause hooks; **seed index/share anchors are now proved lemmas**) without changing existing **theorem statements** (only definitions and proofs behind abstract ops). It aligns **`ComparisonPayloadTypes.ec`**, **`ComparisonPayloadSeedTypes.ec`**, **`ComparisonPayloadFromSeed.ec`**, **`ComparisonPayloadSeedAnchors.ec`**, facade **`ComparisonPayloadSeeds.ec`**, and later **game hops** with **`ms_transcript_observable`** / **`ms_v2_transcript_observable`** (`ms/TranscriptObservable.ec`, `ms/SourceModel.ec`).
