...
| `d_ms3a_real_sim_payload_seed_coupling`, `d_ms3a_coupling_seed_{real,sim}_projection`, `ms3a_real_sim_payload_seed_coupled`, `ms3a_ax_seed_coupling_pair_relation`, `ms3a_ax_seed_support_coupling` | **Definitions** (`ms/source/SourceCouplingTypes.ec`) — joint = `dmap (d_ms3a_seed_spine_joint x s) ms3a_real_sim_seed_pair_of_bitness_layer` (shared spine, **not** the product of `d_ms3a_{real,sim}_payload_seed`); pair predicate as before; **`L_ms3a_ax_seed_coupling_pair_relation_of_spine_support_wf`** proves `ms3a_ax_seed_coupling_pair_relation` from `ms3a_source_wf` on spine support (`SourceCouplingTheorem.ec`); projections fold to `dmap` off the spine (`L_ms3a_coupling_seed_{real,sim}_projection_dmap_spine`); narrow marginal bridge axioms **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_spine_sim_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_spine_marginal_pair_common_lift`** in **`SourcePayloadDistributions.ec`** (**`SourceCouplingAxioms.ec`** summarizes packaging) |
...
| Spine marginal bridges + WF + same-spine lift | **Axioms** `A_ms3a_spine_real_marginal_matches_seed`, `A_ms3a_spine_sim_marginal_matches_seed`, `A_ms3a_seed_spine_support_wf`, `A_ms3a_spine_marginal_pair_common_lift` (`ms/source/SourcePayloadDistributions.ec`) — game-level discharge once `d_ms3a_seed_spine_joint` is instantiated |
...
4. **Common bitness-layer image** — the **equality** of the two seed pushforwards (layer maps off abstract seeds) is lemma **`A_ms3a_bitness_layer_seed_schedule`**, **proved** from the two spine marginal bridge axioms plus `dmap` algebra (still a **proof obligation** at game level to discharge the bridge axioms).
...
`d_ms3a_{real,sim}_payload_seed` and the spine joint can eventually discharge the remaining MS-3a
**axioms** (three public-spine / projection facts for programmed-on-support, four spine bridges, common-lift, WF on spine support), without editing `.ec` proofs here.

### Spine bridge axioms (why they stay axioms here)

The four axioms in **`SourcePayloadDistributions.ec`** are not derivable from the rest of the
`ms/source` development while **`d_ms3a_seed_spine_joint`**, **`d_ms3a_real_payload_seed`**, and
**`d_ms3a_sim_payload_seed`** remain abstract **`op`**s (no `dmap` / `dlet` / `dunit` defining
equations in EasyCrypt). The two **marginal bridge** axioms state **law equalities**; **`A_ms3a_seed_spine_support_wf`**
adds support-side WF; **`A_ms3a_spine_marginal_pair_common_lift`** is a **same-preimage** obligation
for arbitrary pairs on the **abstract** marginal supports — strictly stronger than the two
marginal equalities alone (independent `dmap` preimages for `sr` and `ss` would not force a
common `src`). **No split** of **`A_ms3a_spine_marginal_pair_common_lift`** was introduced: the
four **`A_ms3a_seed_pair_*_source_shared`** proofs require a **single** spine witness for each
pair `(sr, ss)`.
...
They apply to **lists carried by seeds** on abstract `d_ms3a_{real,sim}_payload_seed` support.
...
- **Structured joint (in-repo):** `d_ms3a_real_sim_payload_seed_coupling x s` =
  ...
  **`d_ms3a_seed_spine_joint`** abstract on `ms3a_bitness_layer_source` (`SourcePayloadDistributions.ec`)
  ...
  whereas MS-3a marginal seed laws remain abstract until games discharge the marginal bridge
  axioms `A_ms3a_spine_real_marginal_matches_seed`, `A_ms3a_spine_sim_marginal_matches_seed`
  (and related spine obligations: `A_ms3a_seed_spine_support_wf`, `A_ms3a_spine_marginal_pair_common_lift`).
...
|------------|--------------------------------------------------|
| **`A_ms3a_bitness_layer_seed_schedule`** | **Proved** in `SourceScheduleSeed.ec` from **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_spine_sim_marginal_matches_seed`**, **`dmap_comp`**, **`eq_dmap_in`**. |
| **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_spine_sim_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_spine_marginal_pair_common_lift`** | Instantiate **`d_ms3a_seed_spine_joint`** from games / ROM and prove these equalities / WF / lift facts at the linking layer. |
...
| **Game views / witnesses** | Until `G_MS_*` views expose the actual samplers for bitness-after-*, the **spine bridge** axioms (`A_ms3a_spine_real_marginal_matches_seed`, `A_ms3a_spine_sim_marginal_matches_seed`, `A_ms3a_seed_spine_support_wf`, `A_ms3a_spine_marginal_pair_common_lift`) and the **three** public-spine / seed-projection axioms in `SourceProgrammedObligations.ec` remain **linking obligations** relative to `game_pr`. (The four named `A_ms3a_{real,sim}_seed_{bits,bitness_globals}_programmed_on_support` facts are **proved lemmas** from those three axioms.) The bitness-layer **seed schedule** is already a **proved lemma** (`A_ms3a_bitness_layer_seed_schedule`) from the two marginal bridge axioms at the abstract interface; the four `A_ms3a_seed_pair_*_source_shared` facts are **proved lemmas** from `A_ms3a_spine_marginal_pair_common_lift`. |
...
Abstract **`d_ms3a_{real,sim}_payload_seed`**, **`d_ms3a_seed_spine_joint`**, and the **three** programmed-layer **axioms** (`A_ms3a_public_payload_bitness_programmed`, `A_ms3a_real_seed_bitness_fields_are_public_on_support`, `A_ms3a_sim_seed_bitness_fields_are_public_on_support`) are unchanged; **new** narrow spine obligations (**marginal bridges**, **WF on spine support**, **common-lift**) live in **`SourcePayloadDistributions.ec`**, and **`A_ms3a_bitness_layer_seed_schedule`** / **`A_ms3a_seed_pair_*_source_shared`** are **proved lemmas** at that interface.
...
2. **Joint seed coupling** — **spine phase done:** `d_ms3a_seed_spine_joint` + `dmap` pair
   ...
   **Still missing at games:** (i) instantiate **`d_ms3a_seed_spine_joint`** from execution / ROM;
   (ii) **discharge** those bridge axioms (same marginal equalities as design targets).
...
**MS-3a coupling (immediate):** instantiate **`d_ms3a_seed_spine_joint`** from execution
   (and discharge **`A_ms3a_seed_spine_support_wf`** / **`A_ms3a_spine_marginal_pair_common_lift`** as needed), then **prove** the marginal bridge axioms
   **`A_ms3a_spine_real_marginal_matches_seed`** and **`A_ms3a_spine_sim_marginal_matches_seed`** tying **`dmap d_spine real_of` / `dmap d_spine sim_of`** to
   **`d_ms3a_{real,sim}_payload_seed`**. The four **`A_ms3a_seed_pair_*_source_shared`** lemmas and
   **`A_ms3a_bitness_layer_seed_schedule`** are already **proved** in `ms/source/` from those obligations.

**MS-3a residual:** discharge the **spine** axiom bundle in **`SourcePayloadDistributions.ec`** at games, plus the **three** programmed-layer **axioms** in **`SourceProgrammedObligations.ec`** (public vector programmed + real/sim seed fields agree with public spine on abstract support; the four `A_ms3a_{real,sim}_seed_{bits,bitness_globals}_programmed_on_support` statements are **proved lemmas**); instantiate abstract **`d_ms3a_{real,sim}_payload_seed`** / **`d_ms3a_seed_spine_joint`** from the execution spec / games (`ms3a_*_payload_from_seed` is definitional identity on payload-shaped seeds; **`A_ms3a_seed_pair_public_fields_on_support`** is already a **proved lemma**). Optionally **`ms3a_pack_observable_with_digest_field_correct`** and **`duni_scalar_shift_reparam`**.
