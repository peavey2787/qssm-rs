...
- **MS:** `ms/SourceModel.ec` — abstract observable frame (pack / digest / alignment); **`ms/source/`** — structured source types, constructors, `d_ms3a_*` laws; **MS-3a payload laws** are **`dmap`** pushforwards of **`d_ms3a_real_payload_seed`** (**abstract**) and **`d_ms3a_sim_payload_seed`** (**defined** as the joint sim marginal) through **`ms3a_{real,sim}_payload_from_seed`** (**`SourcePayloadDistributions`** + facade **`SourceDistributions`** / `SourceConstructors` / seed types in **`SourceTypes`**). **`ms3a_payload_real_support_programmed`**, **`ms3a_payload_sim_support_programmed`**, **`ms3a_payload_pair_public_fields_on_support`** are **proved lemmas** from seed support: **`A_ms3a_real_seed_programmed_on_support`** / **`A_ms3a_sim_seed_programmed_on_support`** are **proved lemmas** from four programmed-on-support **lemmas** (**`A_ms3a_{real,sim}_seed_bits_programmed_on_support`**, **`A_ms3a_{real,sim}_seed_bitness_globals_programmed_on_support`**). The real pair use the two remaining **axioms** **`A_ms3a_public_payload_bitness_programmed`** and **`A_ms3a_real_seed_bitness_fields_are_public_on_support`**; the sim pair use **`A_ms3a_public_payload_bitness_programmed`** plus the proved lemma **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`**, together with **`ms3a_{real,sim}_payload_programmed_layer_as_bitness_vector`** in **`SourceConstructors`**. Paired public fields use **proved lemmas** **`A_ms3a_seed_pair_stmt_source_shared`**, **`A_ms3a_seed_pair_res_source_shared`**, **`A_ms3a_seed_pair_comparison_global_source_shared`**, **`A_ms3a_seed_pair_bitness_globals_source_shared`** (by projection from **`A_ms3a_seed_pair_public_fields_match_on_support`** in **`SourcePayloadDistributions.ec`**), **proved lemmata** **`A_ms3a_seed_pair_stmt_on_support`**, **`A_ms3a_seed_pair_res_on_support`**, **`A_ms3a_seed_pair_comparison_global_on_support`**, **`A_ms3a_seed_pair_bitness_globals_on_support`** (payload `from_seed` equalities via **`ms3a_payload_pair_*_eq_from_seed_of_seed_*_eq`** in **`SourceConstructors`**); the bundle is **proved lemma** **`A_ms3a_seed_pair_public_fields_on_support`** (`SourcePublicFieldObligations.ec`, re-exported via **`SourceObligations.ec`**). **Seed types** alias constructor payloads and **`ms3a_*_payload_from_seed`** are the identity (see **`SourceTypes`** / **`SourceConstructors`**). **Status B:** the former broad same-spine witness axiom **`A_ms3a_spine_marginal_pair_common_lift`** was replaced by the narrower support-level public-fields axiom **`A_ms3a_seed_pair_public_fields_match_on_support`**. **Status C:** **`A_ms3a_seed_spine_support_wf`** remains unchanged. `d_ms3a_seed_spine_joint` is still abstract, and **`ms3a_source_wf`** is already the exact invariant consumed by the coupling scaffold: it unfolds directly to **`ms_bitness_vector_programmed_layer`** on the spine record, so no strictly smaller consumed support invariant was isolated this phase. **Status A:** **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`** collapsed to a lemma: sim support inverts definitionally to a spine sample, and the existing real marginal + real projection bridge pins the copied `stmt` / `bits` / `bitness globals` fields to the public spine. **Remaining MS-3a coupling debt** in **`SourcePayloadDistributions.ec`**: three **axioms** — **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_seed_pair_public_fields_match_on_support`** — plus abstract **`d_ms3a_seed_spine_joint`** / **`d_ms3a_real_payload_seed`**; **`d_ms3a_sim_payload_seed`** is **defined** as the joint sim marginal and **`A_ms3a_spine_sim_marginal_matches_seed`** is a **proved** definitional lemma. Games still instantiate the joint and real law and discharge the three axioms; lemma **`A_ms3a_bitness_layer_seed_schedule`** is **proved** from the real axiom + sim lemma (**`SourceScheduleSeed.ec`**, **`dmap_comp`**, **`eq_dmap_in`**); the legacy **`… \o ms3a_*_payload_from_seed`** phrasing is **definitionally equivalent** on seeds (**`L_ms3a_bitness_layer_seed_push_{real,sim}_eq_layer_dmap`**, **`L_ms3a_bitness_layer_seed_schedule_composed_form`**); lemma **`A_ms3a_payload_dmap_bitness_layer_schedule`** is **proved** from **`A_ms3a_bitness_layer_seed_schedule`** + those rewrites + **`ms3a_bitness_{real,sim}_source_as_seed_dmap`** + **`Distr.dmap_comp`**; proved **`ms3a_ax_*`** layer lemmas; legacy compatibility wrapper **`ms3a_payload_schedule_equivalence`** (prefer **`A_ms3a_bitness_layer_seed_schedule`**, lemma **`A_ms3a_payload_dmap_bitness_layer_schedule`**, or **`ms3a_source_eq_from_bitness_layer`** for new work); **`MS_3a_exact_bitness_simulation`**; `ms/MS.ec` — `epsilon_ms_hash_binding`, **`ms1_hash_binding_step`**, **`ms2_rom_programming_step`**, **`ms3a_bitness_exact_step`**, **`ms3b_true_clause_exact_step`** (MS3b: same MS-3b forall bundle as the old game axiom + frozen `GV_ms`, `AfterBitness`→`AfterComparison`; **`Algebra`**, **`List`**, **`TrueClause`** in scope), `A1_ms_hash_binding_nonneg`, **`MS_3c_exact_comparison_simulation`** (wrapper over `ms/Comparison.ec`). ROM budget **`epsilon_ms_rom_programmability`** lives in **`primitives/FS.ec`** with **`A2_ms_rom_programmability_nonneg`** and **`A2_programmable_oracle_exists`**. **MS-3b** in **`ms/TrueClause.ec`** (facade over **`ms/true_clause/TrueClauseTypes.ec`**, **`TrueClauseMSB.ec`**, **`TrueClauseTheorem.ec`**) — structural **`ms3b_comparison_operand_bits`** / **`ms3b_clause_opening_binds`**; explicit MSB-first strict-greater predicate **`ms3b_msb_first_strict_gt_at`**; proved **`A_ms3b_bit_decomposition_correct`**, **`A_ms3b_pedersen_opening_correct`**, **`A_ms3b_highest_differing_bit_correct`**, and lemma **`A_ms3b_comparison_semantics`**; **remaining local semantic debt** = axiom **`A_ms3b_operand_hdb_implies_value_gt_target`** (operands + **`ms_highest_differing_bit`** ⇒ **`ms3b_value_gt_target_at`**: value `true` / target `false` at `p`); lemma **`A_ms3b_operand_hdb_implies_msb_first_strict_gt`** is **proved** from that axiom + **`L_ms3b_hdb_value_gt_target_implies_msb_first_strict_gt`** in **`TrueClauseMSB.ec`**. Still not derivable from list geometry + lengths alone (HDB allows either boolean ordering without the semantic leaf). **MS-3c** in **`ms/Comparison.ec`** — comparison payloads (`d_ms3c_{real,sim}_comparison_payload`), `dmap` schedules, **proved** **`A_ms3c_payload_schedule_equiv`** from a narrower coupling/scheduling layer (`ms3c_real_sim_payload_coupled`, explicit coupling projections `d_ms3c_coupling_{real,sim}_projection`, `ms3c_ax_payload_support_coupling`, and coupling/marginal axioms) plus component payload obligations (see `plans/MS_3c_proof_plan.md`), proved schedule bridge, surface clause laws; **ordered announcement digest list** is **`ms3c_clause_ann_digests_from_surface`** (true then false branch digests via **`ms_single_bit_branch_digest`**), with **`A_ms3c_digest_announcement_only`** stating programmed **`mscc_query_digest`** against that projection only, and true-clause bridge **`A_ms3c_true_clause_from_ms3b_and_schnorr`** now proved from explicit MS-3b hook (`ms3c_true_clause_uses_ms3b_blinder_point`) plus Schnorr reparam readiness (`ms3c_true_clause_reparam_ready` / `MS_3a_single_branch_schnorr_reparam`).
- **MS:** `ms/SourceModel.ec` — abstract observable frame (pack / digest / alignment); **`ms/source/`** — structured source types, constructors, `d_ms3a_*` laws; **MS-3a payload laws** are **`dmap`** pushforwards of **`d_ms3a_real_payload_seed`** (**abstract**) and **`d_ms3a_sim_payload_seed`** (**defined** as the joint sim marginal) through **`ms3a_{real,sim}_payload_from_seed`** (**`SourcePayloadDistributions`** + facade **`SourceDistributions`** / `SourceConstructors` / seed types in **`SourceTypes`**). **`SourceExecutionLink.ec`** is now the standalone execution/public-spine design boundary: predicate **`ms3a_execution_public_spine_link`** packages the exact two remaining bridge targets, and projection lemmas **`ms3a_public_payload_bitness_programmed_of_execution_link`** and **`ms3a_real_seed_public_fields_on_support_of_execution_link`** recover the public-programmed theorem and real-seed/public-spine theorem shape without adding new axiom debt. It depends on **`SourcePayloadDistributions`** + **`SourceModel`** only, so **`SourceProgrammedObligations.ec`** can later import it without creating a cycle; **`SourcePayloadDistributions.ec`** should not import it. **`ms3a_payload_real_support_programmed`**, **`ms3a_payload_sim_support_programmed`**, **`ms3a_payload_pair_public_fields_on_support`** are **proved lemmas** from seed support: **`A_ms3a_real_seed_programmed_on_support`** / **`A_ms3a_sim_seed_programmed_on_support`** are **proved lemmas** from four programmed-on-support **lemmas** (**`A_ms3a_{real,sim}_seed_bits_programmed_on_support`**, **`A_ms3a_{real,sim}_seed_bitness_globals_programmed_on_support`**). The real pair use the two remaining **axioms** **`A_ms3a_public_payload_bitness_programmed`** and **`A_ms3a_real_seed_bitness_fields_are_public_on_support`**; the sim pair use **`A_ms3a_public_payload_bitness_programmed`** plus the proved lemma **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`**, together with **`ms3a_{real,sim}_payload_programmed_layer_as_bitness_vector`** in **`SourceConstructors`**. Paired public fields use **proved lemmas** **`A_ms3a_seed_pair_stmt_source_shared`**, **`A_ms3a_seed_pair_res_source_shared`**, **`A_ms3a_seed_pair_comparison_global_source_shared`**, **`A_ms3a_seed_pair_bitness_globals_source_shared`** (by projection from **`A_ms3a_seed_pair_public_fields_match_on_support`** in **`SourcePayloadDistributions.ec`**), **proved lemmata** **`A_ms3a_seed_pair_stmt_on_support`**, **`A_ms3a_seed_pair_res_on_support`**, **`A_ms3a_seed_pair_comparison_global_on_support`**, **`A_ms3a_seed_pair_bitness_globals_on_support`** (payload `from_seed` equalities via **`ms3a_payload_pair_*_eq_from_seed_of_seed_*_eq`** in **`SourceConstructors`**); the bundle is **proved lemma** **`A_ms3a_seed_pair_public_fields_on_support`** (`SourcePublicFieldObligations.ec`, re-exported via **`SourceObligations.ec`**). **Seed types** alias constructor payloads and **`ms3a_*_payload_from_seed`** are the identity (see **`SourceTypes`** / **`SourceConstructors`**). **Status B:** the former broad same-spine witness axiom **`A_ms3a_spine_marginal_pair_common_lift`** was replaced by the narrower support-level public-fields axiom **`A_ms3a_seed_pair_public_fields_match_on_support`**. **Status C:** **`A_ms3a_seed_spine_support_wf`** remains unchanged. `d_ms3a_seed_spine_joint` is still abstract, and **`ms3a_source_wf`** is already the exact invariant consumed by the coupling scaffold: it unfolds directly to **`ms_bitness_vector_programmed_layer`** on the spine record, so no strictly smaller consumed support invariant was isolated this phase. **Status A:** **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`** collapsed to a lemma: sim support inverts definitionally to a spine sample, and the existing real marginal + real projection bridge pins the copied `stmt` / `bits` / `bitness globals` fields to the public spine. **Remaining MS-3a coupling debt** in **`SourcePayloadDistributions.ec`**: three **axioms** — **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_seed_pair_public_fields_match_on_support`** — plus abstract **`d_ms3a_seed_spine_joint`** / **`d_ms3a_real_payload_seed`**; **`d_ms3a_sim_payload_seed`** is **defined** as the joint sim marginal and **`A_ms3a_spine_sim_marginal_matches_seed`** is a **proved** definitional lemma. Games still instantiate the joint and real law and discharge the three axioms; lemma **`A_ms3a_bitness_layer_seed_schedule`** is **proved** from the real axiom + sim lemma (**`SourceScheduleSeed.ec`**, **`dmap_comp`**, **`eq_dmap_in`**); the legacy **`… \o ms3a_*_payload_from_seed`** phrasing is **definitionally equivalent** on seeds (**`L_ms3a_bitness_layer_seed_push_{real,sim}_eq_layer_dmap`**, **`L_ms3a_bitness_layer_seed_schedule_composed_form`**); lemma **`A_ms3a_payload_dmap_bitness_layer_schedule`** is **proved** from **`A_ms3a_bitness_layer_seed_schedule`** + those rewrites + **`ms3a_bitness_{real,sim}_source_as_seed_dmap`** + **`Distr.dmap_comp`**; proved **`ms3a_ax_*`** layer lemmas; legacy compatibility wrapper **`ms3a_payload_schedule_equivalence`** (prefer **`A_ms3a_bitness_layer_seed_schedule`**, lemma **`A_ms3a_payload_dmap_bitness_layer_schedule`**, or **`ms3a_source_eq_from_bitness_layer`** for new work); **`MS_3a_exact_bitness_simulation`**; `ms/MS.ec` — `epsilon_ms_hash_binding`, **`ms1_hash_binding_step`**, **`ms2_rom_programming_step`**, **`ms3a_bitness_exact_step`**, **`ms3b_true_clause_exact_step`** (MS3b: same MS-3b forall bundle as the old game axiom + frozen `GV_ms`, `AfterBitness`→`AfterComparison`; **`Algebra`**, **`List`**, **`TrueClause`** in scope), `A1_ms_hash_binding_nonneg`, **`MS_3c_exact_comparison_simulation`** (wrapper over `ms/Comparison.ec`). ROM budget **`epsilon_ms_rom_programmability`** lives in **`primitives/FS.ec`** with **`A2_ms_rom_programmability_nonneg`** and **`A2_programmable_oracle_exists`**. **MS-3b** in **`ms/TrueClause.ec`** (facade over **`ms/true_clause/TrueClauseTypes.ec`**, **`TrueClauseMSB.ec`**, **`TrueClauseTheorem.ec`**) — structural **`ms3b_comparison_operand_bits`** / **`ms3b_clause_opening_binds`**; explicit MSB-first strict-greater predicate **`ms3b_msb_first_strict_gt_at`**; proved **`A_ms3b_bit_decomposition_correct`**, **`A_ms3b_pedersen_opening_correct`**, **`A_ms3b_highest_differing_bit_correct`**, and lemma **`A_ms3b_comparison_semantics`**; **remaining local semantic debt** = axiom **`A_ms3b_operand_hdb_implies_value_gt_target`** (operands + **`ms_highest_differing_bit`** ⇒ **`ms3b_value_gt_target_at`**: value `true` / target `false` at `p`); lemma **`A_ms3b_operand_hdb_implies_msb_first_strict_gt`** is **proved** from that axiom + **`L_ms3b_hdb_value_gt_target_implies_msb_first_strict_gt`** in **`TrueClauseMSB.ec`**. Still not derivable from list geometry + lengths alone (HDB allows either boolean ordering without the semantic leaf). **MS-3c** in **`ms/Comparison.ec`** — comparison payloads (`d_ms3c_{real,sim}_comparison_payload`), `dmap` schedules, **proved** **`A_ms3c_payload_schedule_equiv`** from a narrower coupling/scheduling layer (`ms3c_real_sim_payload_coupled`, explicit coupling projections `d_ms3c_coupling_{real,sim}_projection`, `ms3c_ax_payload_support_coupling`, and coupling/marginal axioms) plus component payload obligations (see `plans/MS_3c_proof_plan.md`), proved schedule bridge, surface clause laws; **ordered announcement digest list** is **`ms3c_clause_ann_digests_from_surface`** (true then false branch digests via **`ms_single_bit_branch_digest`**), with **`A_ms3c_digest_announcement_only`** stating programmed **`mscc_query_digest`** against that projection only, and true-clause bridge **`A_ms3c_true_clause_from_ms3b_and_schnorr`** now proved from explicit MS-3b hook (`ms3c_true_clause_uses_ms3b_blinder_point`) plus Schnorr reparam readiness (`ms3c_true_clause_reparam_ready` / `MS_3a_single_branch_schnorr_reparam`).
...
**Status C:** **`A_ms3a_public_payload_bitness_programmed`** also remains unchanged. It is the root public-spine `ms_bitness_vector_programmed_layer` fact consumed by the real/sim seed-support lemmas via `MS_3a_all_bits_from_single_bit`, and the current EasyCrypt layer provides only uninterpreted public projections plus length/digest shape predicates, not the execution / ROM / FS semantics needed to prove or meaningfully narrow that programmed predicate.
**Execution/public-payload linkage audit:** local MS-3a narrowing is now mostly exhausted. The remaining programmed debt is **execution/game linkage**, not another source-local rewrite: `G_MS_*` in [games/GameViews.ec](games/GameViews.ec) only freeze `xms` and use `witness` for `msgv_ms_obs`; [games/GameMSHopTypes.ec](games/GameMSHopTypes.ec) and [games/GameMSHopTransitions.ec](games/GameMSHopTransitions.ec) package stage premises and hop bounds, not transcript/public-input generation; [games/GameAdvantage.ec](games/GameAdvantage.ec) keeps `game_pr` abstract; [ms/MS.ec](ms/MS.ec) and [sim/Simulator.ec](sim/Simulator.ec) expose only transcript shells. The smallest future bridge package is therefore: (1) a theorem that the canonical public spine `ms3a_public_stmt_digest` / `ms3a_public_bits` / `ms3a_public_bitness_globals` forms a programmed bitness vector, discharging **`A_ms3a_public_payload_bitness_programmed`**; and (2) a theorem or concrete marginal law showing every supported real seed matches those same public fields, discharging **`A_ms3a_real_seed_bitness_fields_are_public_on_support`** and naturally feeding the real marginal bridge if stated at marginal level.
...
**Execution/link boundary:** **`SourceExecutionLink.ec`** is the new standalone skeleton theory for the remaining MS-3a execution/public-spine debt. It defines predicate **`ms3a_execution_public_spine_link`** and the proved projection lemmas **`ms3a_public_payload_bitness_programmed_of_execution_link`** and **`ms3a_real_seed_public_fields_on_support_of_execution_link`**. The intended future dependency direction is: concrete execution/game linkage proves **`ms3a_execution_public_spine_link`**; `SourceProgrammedObligations.ec` may later import this theory to discharge its two remaining programmed-layer axioms; `SourcePayloadDistributions.ec` must remain below it to avoid cycles.

1. **MS-3a (residual)** — discharge the **source** axiom bundle (still **three** axioms: **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_seed_pair_public_fields_match_on_support`**) and the **two** remaining programmed-layer **axioms** in **`SourceProgrammedObligations.ec`** (concretization audit: **`plans/MS_3a_proof_plan.md`** → section **“Seed distribution concretization (design audit)”**): **`A_ms3a_public_payload_bitness_programmed`**, **`A_ms3a_real_seed_bitness_fields_are_public_on_support`**. **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`** is now a **proved lemma** from the definitional sim marginal plus the existing real marginal / real projection bridge. The next useful step is no longer another local MS-3a proof shave; it is to prove **`ms3a_execution_public_spine_link`** in **`SourceExecutionLink.ec`**, whose two projection lemmas are already the exact theorem shapes needed to discharge the two remaining programmed-layer axioms. The four `A_ms3a_{real,sim}_seed_{bits,bitness_globals}_programmed_on_support` names remain **proved lemmas** packaged by **`A_ms3a_{real,sim}_seed_programmed_on_support`**; instantiate abstract **`d_ms3a_seed_spine_joint`** / **`d_ms3a_real_payload_seed`** from execution/games (**`d_ms3a_sim_payload_seed`** is already the joint sim marginal **by definition**; lemma **`A_ms3a_spine_sim_marginal_matches_seed`** is proved); payload **`ms3a_*_payload_from_seed`** is definitional identity on payload-shaped seeds; the four **`A_ms3a_seed_pair_*_source_shared`** facts and **`A_ms3a_bitness_layer_seed_schedule`** are already **proved lemmas** at the abstract interface; **`A_ms3a_seed_pair_public_fields_on_support`** is already a **proved lemma**; see `plans/MS_3a_proof_plan.md`.
