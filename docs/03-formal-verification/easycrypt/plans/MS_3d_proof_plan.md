...
**Initialization status:** lower MS1/MS2 bridge installation is complete, and the MS1 half is now discharged. **`games/GameAdvantage.ec`** now defines **`game_pr_ms_core`** from **`ms_view_distinguish_pr (d_ms_game_stage_observable_v2 ...)`** and proves **`A_MS1_hash_binding_game_pr_core_bound`** as a lemma. **`A_MS2_rom_programming_game_pr_core_bound`** remains the only MS1/MS2 lower bridge axiom there. In **`games/GameMSHopTypes.ec`**, **`A_MS1_hash_binding_concrete_pair_advantage_bound`** and **`A_MS2_rom_programming_concrete_pair_advantage_bound`** remain proved lemmas, and the step/canonical bounds remain proved lemmas above them. The remaining proof debt is now narrower: only the MS2 lower bridge axiom is still not reducible to the current repo assumptions in **`ms/MS.ec`**, **`primitives/FS.ec`**, or **`theorem/MainTheorem.ec`**.
...
- **Stable projection layer:** **`games/GameAdvantage.ec`** now owns **`ms3a_game_pr_stage`**, **`ms3b_game_pr_stage`**, **`ms3c_game_pr_stage`**, **`game_pr`**, **`Adv`**, **`A_adv_ms_hop_telescope`**, the concrete MS probability core **`game_pr_ms_core`**, the proved MS1 bridge lemma **`A_MS1_hash_binding_game_pr_core_bound`**, and the remaining MS2 bridge axiom **`A_MS2_rom_programming_game_pr_core_bound`**.
...
- **Debt reduction already achieved:** the MS1/MS2 game-layer axiom surface in **`games/GameMSHopTypes.ec`** dropped from six provisional axioms to zero: the explicit concrete stage-pair theorems, the generic step-bound surfaces, and the canonical pair bounds are now all lemmas. At the lower **`game_pr_ms_core`** boundary in **`games/GameAdvantage.ec`**, MS1 is also now discharged; the only remaining MS1/MS2 axiom there is the MS2 bridge.
- **Composed MS transition theorem:** **`A_G0_to_G1_ms_transition_bound`** is already a proved lemma in **`games/GameMSHopComposition.ec`**. Its remaining MS1/MS2 assumption debt now flows only through the lower MS2 bridge axiom in **`games/GameAdvantage.ec`**.
...
This means the correct MS-3d direction is now even narrower: finalize the lower MS boundary by discharging the surviving MS2 **`game_pr_ms_core`** bridge axiom, not by adding new MS-3c structure or by widening the comparison-side execution boundary.
...
- **Smallest missing lower objects:**
	...
	- **Still missing theorem debt:** the sampled MS2 stage law is not yet connected to a lower cryptographic bound.
...
- **Why MS2 is still blocked:** **`game_pr_ms_core`** is now concrete, but the existing ROM surfaces still only assert **`0%r <= epsilon_ms_rom_programmability`** plus **`A2_programmable_oracle_exists`** for scalar response existence at a ROM query point. The remaining lower axiom **`A_MS2_rom_programming_game_pr_core_bound`** is therefore the current semantic boundary rather than a proved lemma.
...
4. **Current blocker:** discharge **`A_MS2_rom_programming_game_pr_core_bound`** from concrete cryptographic semantics below **`game_pr_ms_core`**, then re-check **`A_G0_to_G1_ms_transition_bound`** with no remaining MS1/MS2 axioms anywhere in the game layer.
	The newly added **`ms/MSProbabilitySurface.ec`** is now the active lower entry point for MS semantics inside **`game_pr_ms_core`**. The next proof obligation is **`A_MS2_rom_programming_transition_bound`**, so that the remaining MS2 axiom can be removed the same way MS1 was.
...
- Remaining MS-side game-layer assumption debt reduced to the single lower **`game_pr_ms_core`** cryptographic bridge axiom for MS2, or that axiom fully discharged.
