...
use qssm_api::{ProofBundle, ProofContext};
...
fn same_entropy_seed_produces_identical_ms_root() {
    ...
    let b = binding();
    let proof1 = prove(&ProofContext::new(seed()), &template(), &claim(), 100, 50, b, ent)
        .expect("prove 1");
    let proof2 = prove(&ProofContext::new(seed()), &template(), &claim(), 100, 50, b, ent)
        .expect("prove 2");
