...
| Medium | Concrete LE Set B constants, challenge-expansion details, attempt bounds, and numeric security floors are not embedded as current EasyCrypt constants or numeric theorem outputs | `qssm-le-engine-a.md`, `security/ASSUMPTION_ANALYSIS.md`, `security/SECURITY_MODEL_MAP.md` | `le/*.ec` surfaces are symbolic / predicate-based; `formal/SECURITY_INSTANTIATION.md` is explanatory, not a concrete-constant proof | The docs now make this boundary explicit; residual work remains only if constant-level machine-checked refinement is desired | Docs clarification + optional refinement work |
