...
This note tracks **exact comparison-clause simulation** under programmed Fiat–Shamir. The **`ms/comparison/*.ec`** tree is **axiom-free** (all former **`A_ms3c_*`** obligations there are **proved lemmata**). Remaining MS-3c **game-hop** debt is the single axiom **`A_MS3c_comparison_bundle_implies_game_pr_equality`** in **`games/GameMSHopTypes.ec`**, which links the MS-3c implication bundle to equality of abstract **`game_pr`** on **`G_MS_after_comparison`** vs **`G_MS_sim`**; the historically named bound **`A_MS3c_canonical_comparison_exact_bound`** is now a **proved lemma** (same statement) from **`Adv_def`** once that bridge holds.

**MS-3b handoff status:** the canonical MS-3b game-layer bound is already proved, but the last MS-3b semantic leaf **`A_ms3b_operand_hdb_implies_value_gt_target`** is still open because the current MS-3b predicates do not yet read concrete comparison operands or true-clause openings from `ms_public_input` or `ms_transcript_observable`. The new lemmas in `ms/true_clause/TrueClauseTheorem.ec` make that explicit: the current operand/opening predicates ignore `x`, and there is a highest-differing-bit counterexample with the opposite direction. So MS-3c should now focus on concrete comparison payload and transcript carriers rather than expecting that remaining MS-3b leaf to disappear without a carrier refinement.
...
| **`A_MS3c_comparison_bundle_implies_game_pr_equality`** | **axiom** (`games/GameMSHopTypes.ec`) | The MS-3c implication bundle (same shape as **`ms3c_comparison_exact_step`**) ⇒ **`game_pr (G_MS_after_comparison …) D = game_pr (G_MS_sim …) D`** for all distinguishers **`D`**. Execution / observable semantics gap: schedule equality (**`ms_comparison_exact_simulation_equiv`**) is proved under **`ms/comparison/`**, but **`game_pr`** is still abstract. |
| **`A_MS3c_canonical_comparison_exact_bound`** | **lemma** (`games/GameMSHopTypes.ec`) | Same bundle as the axiom above ⇒ **`Adv (G_MS_after_comparison …) (G_MS_sim …) D <= 0%r`**; proved from **`Adv_def`**, **`A_MS3c_comparison_bundle_implies_game_pr_equality`**, and **`ring`**. |
...
| Risk | Mitigation |
|------|------------|
| **MS-3b true-clause hook still sits on abstract operand/opening predicates** | Refine `ms_public_input`, `ms_transcript_observable`, or a dedicated comparison-slice carrier so MS-3b can decode operand bits and a true-clause opening concretely; then feed those fields into the MS-3c true-clause hook instead of the current abstract parameters. |
...
**Expected to remain axiomatic after early patches:** properties true in the execution spec but not forced by definitions (ROM collision resistance, the remaining MS-3b operand-direction leaf, transcript-level announcement algebra) stay as named axioms until constructor/transcript wiring discharges them in bulk. On the **game** side, **`A_MS3c_comparison_bundle_implies_game_pr_equality`** is the narrow **`game_pr`** bridge for the MS3c hop until **`game_pr`** is defined from concrete modules and shown insensitive to **`msgv_stage`** between **`MSGameStageAfterComparison`** and **`MSGameStageSim`** when schedules match.

## Next target

See **§ Concrete MS-3c payload constructor design** above: prefer **richer seed types + `from_seed` + component laws**, keeping **`stmt := ms3c_public_stmt_digest x`** for **`mscp_query_digest`** (digest constructor is **proved** for Phase-1; no `forall stmt`). Before the remaining MS-3b leaf can disappear, those richer carriers also need to expose concrete comparison operands and the true-clause opening that MS-3b currently quantifies abstractly. In parallel: thread **`ms_transcript_observable`** through **`games/GameViews.ec`** / hop modules; replace **`witness`** in **`ms3c_public_stmt_digest`** with transcript-backed statement material. **Game hop:** discharge **`A_MS3c_comparison_bundle_implies_game_pr_equality`** (or refine **`game_pr`** so the bridge becomes definitional); **`A_MS3c_canonical_comparison_exact_bound`** is already a **lemma** layered on that axiom.
