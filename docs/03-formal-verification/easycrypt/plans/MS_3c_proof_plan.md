...
**Expected to remain axiomatic after early patches:** properties true in the execution spec but not forced by definitions (ROM collision resistance, MS-3b operand hooks, transcript-level announcement algebra) stay as named axioms until constructor/transcript wiring discharges them in bulk. On the **game** side, **`A_MS3c_comparison_bundle_implies_game_pr_equality`** is the narrow **`game_pr`** bridge for the MS3c hop until **`game_pr`** is defined from concrete modules and shown insensitive to **`msgv_stage`** between **`MSGameStageAfterComparison`** and **`MSGameStageSim`** when schedules match.
