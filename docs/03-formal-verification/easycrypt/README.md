...
- **Theorem:** `theorem/MainTheorem.ec` — remaining non-negativity placeholders `A1_ms_hash_binding` and `A2_ms_rom_programmability`, theorem-local lemma `A4_le_hvzk` derived from `A4_le_hvzk_bound_nonneg`, bridge lemmas `use_MS_3a` / `use_MS_3b` / `use_MS_3c`, and proved additive game-hop lemma `qssm_main_theorem_skeleton`, now specialized to `extract_ms_public x` rather than a free unrelated `xms`
