...
- **MS:** `SourceExecutionLink.ec` remains the source-facing execution/public-spine boundary, and **`SourceRealExecutionSeed.ec`** now centralizes the remaining execution-seed semantic debt in **`A_ms3a_real_payload_seed_matches_execution_seed`** and **`A_ms3a_real_execution_seed_link`**. The former programmed-layer axiom names in `SourceProgrammedObligations.ec` are now lemmas, and the net named MS-3a axiom count is **5**.
...
**Execution/public-payload linkage status:** the remaining programmed/public-spine semantic debt is now centralized in **`ms/source/SourceRealExecutionSeed.ec`**, not spread across `SourceProgrammedObligations.ec`. The concrete boundary law **`d_ms3a_real_execution_public_seed`**, the bridge axiom **`A_ms3a_real_payload_seed_matches_execution_seed`**, and the package axiom **`A_ms3a_real_execution_seed_link`** are the only MS-3a execution-seed assumptions needed to recover the old theorem shapes downstream.
**Real execution-seed boundary:** `SourceRealExecutionSeed.ec` now packages the two execution-facing facts in **`ms3a_real_execution_seed_link`**, proves the projection lemmas **`ms3a_public_payload_bitness_programmed_of_real_execution_seed_link`** and **`ms3a_real_seed_public_fields_on_support_of_real_execution_seed_link`**, bridges them back to the old abstract real law through **`A_ms3a_real_payload_seed_matches_execution_seed`**, and exports theorem-shape lemmas **`ms3a_public_payload_bitness_programmed_of_execution_seed_law`**, **`ms3a_real_seed_public_fields_on_support_of_execution_seed_law`**, and **`ms3a_execution_public_spine_link_of_execution_seed_law`**. Net named MS-3a axiom count is now **5**: the three spine axioms in **`SourcePayloadDistributions.ec`** plus **`A_ms3a_real_payload_seed_matches_execution_seed`** and **`A_ms3a_real_execution_seed_link`** in **`SourceRealExecutionSeed.ec`**.
**Real-seed wiring audit:** the bridge keeps **`d_ms3a_real_payload_seed`** as the exact-distribution surface seen by **`SourceBitnessDistributions.ec`**, **`SourceScheduleSeed.ec`**, and **`SourceCouplingTypes.ec`**, while support/projection consumers use the derived execution-seed lemmas through `SourceProgrammedObligations.ec`. No import cycle was introduced when `SourceProgrammedObligations.ec` imported `SourceRealExecutionSeed.ec`; the next non-local step is to justify **`A_ms3a_real_execution_seed_link`** from execution/game semantics rather than re-open source-local rewrites.
...
**Concrete boundary on top:** **`SourceRealExecutionSeed.ec`** now carries the concrete execution-seed law, the bridge axiom **`A_ms3a_real_payload_seed_matches_execution_seed`**, and the centralized package axiom **`A_ms3a_real_execution_seed_link`**. `SourceProgrammedObligations.ec` already imports this theory and proves the former programmed-layer axiom names as lemmas. The next useful patch is to replace the remaining package axiom **`A_ms3a_real_execution_seed_link`** with an execution/game theorem.

1. **MS-3a (residual)** — discharge the **source** axiom bundle (still **three** axioms: **`A_ms3a_spine_real_marginal_matches_seed`**, **`A_ms3a_seed_spine_support_wf`**, **`A_ms3a_seed_pair_public_fields_match_on_support`**) plus the **two** execution-seed axioms now centralized in **`SourceRealExecutionSeed.ec`**: **`A_ms3a_real_payload_seed_matches_execution_seed`** and **`A_ms3a_real_execution_seed_link`**. **`A_ms3a_public_payload_bitness_programmed`**, **`A_ms3a_real_seed_bitness_fields_are_public_on_support`**, and **`A_ms3a_sim_seed_bitness_fields_are_public_on_support`** are all now **proved lemmas**. The next useful step is no longer another local MS-3a proof shave; it is to justify **`A_ms3a_real_execution_seed_link`** in the execution/game layer so the centralized package in `SourceRealExecutionSeed.ec` is no longer axiomatic. The four `A_ms3a_{real,sim}_seed_{bits,bitness_globals}_programmed_on_support` names remain **proved lemmas** packaged by **`A_ms3a_{real,sim}_seed_programmed_on_support`**; instantiate abstract **`d_ms3a_seed_spine_joint`** / **`d_ms3a_real_payload_seed`** from execution/games (**`d_ms3a_sim_payload_seed`** is already the joint sim marginal **by definition**; lemma **`A_ms3a_spine_sim_marginal_matches_seed`** is proved); payload **`ms3a_*_payload_from_seed`** is definitional identity on payload-shaped seeds; the four **`A_ms3a_seed_pair_*_source_shared`** facts and **`A_ms3a_bitness_layer_seed_schedule`** are already **proved lemmas** at the abstract interface; **`A_ms3a_seed_pair_public_fields_on_support`** is already a **proved lemma**; see `plans/MS_3a_proof_plan.md`.
