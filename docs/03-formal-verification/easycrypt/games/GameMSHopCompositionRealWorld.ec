...
require import MSProbabilitySurfaceRealWorld.
...
(* Parallel real-world MS composition surface.
   This keeps the exact-zero MS3a/MS3b/MS3c segment route unchanged while
   replacing the concrete parameterized owner constants with explicit abstract
   real-world budget obligations on the live MS1/MS2 lower masses. *)

lemma A_G0_to_G1_ms_realworld_transition_bound
  (b : realworld_budget) :
  forall (x : qssm_public_input) (xms : ms_public_input) (s : seed)
    ...
    Adv_G0_G1_MS x xms s D <=
      ...
      epsilon_ms_rom_programmability_realworld b.
proof.
move=> x xms s D Hrw H3a H3b H3c.
...
have H1 : Adv (G_MS_real x xms s) (G_MS_after_binding x xms s) D <=
    epsilon_ms_hash_binding_realworld b.
  rewrite /G_MS_real /G_MS_after_binding /G0_real_qssm /mk_ms_game_view /=.
  exact (A_MS1_hash_binding_realworld_bad_event_bound b x s xms D Hrw).
have H2 : Adv (G_MS_after_binding x xms s) (G_MS_after_rom x xms s) D <=
    epsilon_ms_rom_programmability_realworld b +
    epsilon_ms_rom_programmability_realworld b.
  rewrite /G_MS_after_binding /G_MS_after_rom /mk_ms_game_view /=.
  exact (A_MS2_canonical_rom_programming_realworld_bound b x s xms D Hrw).
