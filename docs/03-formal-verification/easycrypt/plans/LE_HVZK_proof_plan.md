...
Parameterization freeze checkpoint, May 2026: the LE parameterized lane is now the only parameterized theorem route that closes at the top level. `GameLEBridgeParameterized.ec` feeds `theorem/MainTheoremParameterized.ec : qssm_main_theorem_le_parameterized_budget`, and that theorem intentionally keeps the canonical/demo MS contribution unchanged while parameterizing only the LE side. No full canonical `qssm_main_theorem_parameterized_budget` is claimed: the MS public AfterRom endpoint is only budget-close to the canonical AfterRom endpoint, not zero-equal, so the staged/public-endpoint MS parameterized lane remains below the theorem surface.
