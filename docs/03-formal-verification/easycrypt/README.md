...
- The semantic rejection lane is now wired in parallel to the semantic theorem path through the local experiment quantity `LERejectionSampler.le_rejection_shadow_semantic_failure_probability`, the owned budget `epsilon_le_rej_semantic = mu1 d_le_rejection_semantic_branch_choice true = 1%r / 4%r`, and the semantic umbrella `epsilon_le_semantic = epsilon_le_rej_semantic + epsilon_le_fs_semantic = 3%r / 4%r`, while the exact-zero rejection route still remains unchanged.
- The next meaningful modeling refinement is to replace the current primitive-owned semantic rejection branch experiment with a lower execution-owned rejection model and thread that semantic post image into the semantic comparison path without disturbing the exact-zero theorem path or the public theorem names.