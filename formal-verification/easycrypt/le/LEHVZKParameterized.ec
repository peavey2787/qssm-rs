...
require import LEStatisticalDistanceParameterized.

lemma A_LE_SetB_HVZK_semantic_parameterized_budget_bound :
...
exact (A_LE_semantic_view_advantage_bound_from_parameterized_budget x s D Hrealdef Hsimdef Hind Heps).
qed.

lemma A_LE_HVZK_semantic_parameterized_budget_transition_bound :
  forall (x : qssm_public_input) (s : seed) (D : distinguisher),
...
exact (A_LE_SetB_HVZK_semantic_parameterized_budget_bound x s D Hsetb Heps Hfs).
qed.