...
require import QssmTypes FS BitnessOne.

(* MS v2 bitness across all bit indices (width 64).
   Lifts BitnessOne (one-bit OR + programmed FS). List order models
...
(* Bridge to `ms_bitness_global_challenges` (abstract transcript surface in
   `ms/SourceModel.ec`): ordered digest list. *)
