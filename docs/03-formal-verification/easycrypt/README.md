...
- The LE FS shadow surface now has two checker-validated internal lanes: an exported projected-post lane that still proves the theorem-facing zero-budget bridge lemmas, and a separate shadow-local semantic branch experiment with a genuine two-branch good/bad sampler. Both semantic branches have support, and the semantic failure probability is tied to the sampler-local bad-branch mass rather than to `epsilon_le_fs`.
