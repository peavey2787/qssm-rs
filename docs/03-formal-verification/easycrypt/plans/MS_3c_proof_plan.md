...
**Still open:** Phase-1 comparison data and seed carriers are native, the component laws sample latent ROM/transcript scalar coins, and the coupling layer no longer depends on collapsing paired payloads to the same support point. The next MS-3c work is to let those sampled coins affect payload-visible transcript/ROM fields and then strengthen the hook proofs in **`ComparisonCouplingAxioms.ec`** so they no longer lean on the current support-local Phase-1 collapse. The **`A_ms3c_{real,sim}_from_seed_uses_*`** facts are now **support-local proved lemmas** in **`ComparisonPayloadSeedAnchors.ec`**, all four component samplers remain lossless, and the remaining proof debt is therefore in non-degenerate payload / execution modeling, not in placeholder or unit comparison carriers.
