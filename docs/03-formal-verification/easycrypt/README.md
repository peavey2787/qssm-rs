...
The main live refinement track is the LE shadow lane. Rejection and FS theorem-facing component bounds already route through lower shadow surfaces, and the branch-sensitive FS shadow post constructor is now landed and checker-validated on the current exact-zero model. The lower FS surface now splits into two internal lanes. The exported projected-post lane still proves the theorem-facing bridge theorems consumed by `LEFsProgramming.ec`, keeps the active FS endpoint unchanged, and still closes with `epsilon_le_fs = 0%r`. Beside it, `LEFsProgrammingSurface.ec` now carries a shadow-local semantic branch experiment with a genuine two-branch good/bad sampler, semantic post marginal, and local bad-branch mass that now closes in owned form to `bad_slot_count%r / total_slot_count%r`, which instantiates in the current model to `1%r / 2%r`. The semantic failure probability on that local lane closes to the same owned ratio. The new branch-image layer makes the split explicit: the good semantic branch image matches the theorem-facing programmed view, the bad semantic branch image is a separate local semantic programmed post view, the semantic-post marginal is the branch-choice pushforward over those two images, and its distance from the theorem-facing programmed view is locally bounded by the same bad-branch mass. The rejection shadow lane now also exports a local semantic rejection failure quantity `LERejectionSampler.le_rejection_shadow_failure_probability`, together with an owned semantic rejection budget `epsilon_le_rej_semantic`; on the current carrier that semantic rejection component still closes to `0%r`. `BudgetParameters.ec` now owns the semantic FS branch law itself via primitive-owned slot counts: `total_slot_count = 4`, `bad_slot_count = 2`, `le_fs_semantic_branch_slot_support = range 0 total_slot_count`, `le_fs_semantic_bad_branch_slot slot = slot < bad_slot_count`, `d_le_fs_semantic_branch_slot_choice = duniform le_fs_semantic_branch_slot_support`, and `d_le_fs_semantic_branch_choice = dmap d_le_fs_semantic_branch_slot_choice le_fs_semantic_bad_branch_slot`. Those slot counts are intentionally concrete demo/proof parameters for the current semantic FS experiment, not values sourced from a protocol-parameter bundle. Any future move to a dedicated `primitives/ProtocolParameters.ec` leaf is deferred until there is a real shared protocol parameter surface worth centralizing. The owned semantic FS budget remains `epsilon_le_fs_semantic = mu1 d_le_fs_semantic_branch_choice true`, and its closed form is now `bad_slot_count%r / total_slot_count%r`, which in the current model still evaluates to `1%r / 2%r`. The semantic umbrella LE budget now uses both semantic components: `epsilon_le_semantic = epsilon_le_rej_semantic + epsilon_le_fs_semantic`. `LERejectionSampler.ec` proves the local rejection failure probability is bounded by `epsilon_le_rej_semantic`, `LEFsProgrammingSurface.ec` proves the local bad-branch mass equals the owned FS budget, and the theorem-facing semantic chain now closes both at the local-mass level and at the owned component-sum / umbrella levels. The public theorem surface is now normalized: cite `qssm_main_theorem` for the exact-zero abstraction theorem, and cite `qssm_main_theorem_semantic_budget` for the nonzero semantic-budget theorem. `qssm_main_theorem_nonzero_budget` is a façade alias to the same semantic umbrella theorem, while `qssm_main_theorem_semantic_budget_local_mass`, `qssm_main_theorem_semantic_budget_owned`, and `qssm_main_theorem_semantic_budget_umbrella` are retained as comparison or compatibility lemmas. `A_G1_to_G2_le_transition_bound` and `qssm_main_theorem_skeleton` remain unchanged, so the exact-zero model stays live beside the semantic budget path.
...
- The semantic rejection lane is now wired in parallel to the semantic theorem path through `LERejectionSampler.le_rejection_shadow_failure_probability`, `epsilon_le_rej_semantic`, and the semantic umbrella `epsilon_le_semantic = epsilon_le_rej_semantic + epsilon_le_fs_semantic`, while the exact-zero rejection route remains unchanged.
- The next meaningful modeling refinement is to make that semantic rejection lane nontrivial: replace the current zero closed form with a genuine non-identity rejection sampler or equivalent lower failure model, without disturbing the exact-zero theorem path or the public theorem names.