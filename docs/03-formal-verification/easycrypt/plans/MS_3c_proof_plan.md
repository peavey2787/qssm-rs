...
Decision for this phase: keep the auxiliary coin-driven ROM/transcript views inside the comparison lane. They are no longer merely hypothetical: they now feed the comparison-side execution-seed package in **`ComparisonPayloadSeedTypes.ec`** / **`ComparisonPayloadFromSeed.ec`**. What remains deferred is threading that package into broader source/game state, which would require wider non-public state interfaces.
...
See **§ Concrete MS-3c payload constructor design** above: prefer **richer seed types + `from_seed` + component laws**, keeping **`stmt := ms3c_public_stmt_digest x`** for **`mscp_query_digest`** (digest constructor is **proved** for Phase-1; no `forall stmt`). The current phase has replaced raw placeholders and digest-backed comparison openings with a native comparison slice on **`ms_public_input`**, native comparison openings on **`ms_transcript_observable`**, and explicit game-layer alignment lemmas for the structured Phase-1 seed/payload surface. The comparison-side execution-seed package modeled on **`SourceRealExecutionSeed.ec`** is now implemented and is the active non-public consumer for the auxiliary ROM row and transcript-opening bundle. The next concrete target is to decide whether that package should stay comparison-local or be threaded upward into broader source/game execution state. **Game hop:** no additional MS-3c bridge axiom remains; **`A_MS3c_comparison_bundle_implies_game_pr_equality`** and **`A_MS3c_canonical_comparison_exact_bound`** are both proved lemmas.
