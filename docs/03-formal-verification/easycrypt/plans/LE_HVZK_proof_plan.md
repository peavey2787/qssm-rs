...
Concrete external-bound instantiation checkpoint, May 2026: `RealWorldBudgetInstantiation.ec` now packages `realworld_budget_concrete_128`, proves the component budget `1 / 2^98` and the top closed form `5 / 2^98`, and adds `qssm_main_theorem_realworld_concrete_128` plus `qssm_main_theorem_realworld_concrete_128_5_over_2_98` as concrete external-bound companions to `qssm_main_theorem_realworld_budget`. Those concrete theorems still require four explicit component-bound premises, do not internally prove those reductions, keep the duplicated MS2 charge explicit, preserve the public AfterRom caveat, and do not introduce weighted/non-uniform sampler semantics.
