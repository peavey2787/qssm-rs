...
- **Why MS1 is still blocked:** **`game_pr_ms_core`** in **`games/GameAdvantage.ec`** is still an abstract operator, **`distinguisher`** in **`primitives/QssmTypes.ec`** is abstract, and the existing hash-binding surfaces only assert **`0%r <= epsilon_ms_hash_binding`**. The new lower axiom **`A_MS1_hash_binding_game_pr_core_bound`** is therefore the current semantic boundary rather than a proved lemma.
