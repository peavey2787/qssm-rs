...
- **LE bridge interface:** `games/GameLEBridge.ec` now proves `A_game_pr_LE_projection_semantics` as a **lemma** on the split LE-facing views `G1_le_real_projection` / `G2_full_sim`; the remaining exposed cross-layer gap is the explicit intermediate hop `Adv_G1_MS_to_LE` between `G_MS_sim` and `G1_le_real_projection`, which is carried as a theorem-level obligation rather than a game-layer axiom.
- **Simulator:** `sim/Simulator.ec` — `simulate_qssm_transcript_public_only`
- **Types / games:** `primitives/QssmTypes.ec` now provides the primitive domains plus the concrete **`ms_transcript_observable`** carrier, while **`ms/SourceTypes.ec`** defines **`ms_public_input`**, **`ms_game_stage`**, **`ms_game_view_record`**, **`qssm_g1_le_real_record`**, **`qssm_g2_shell_record`**, and **`game_view`**. The game layer remains split under `games/`: `GameTypes.ec` (MS view helpers/stage predicates), `GameViews.ec` (G0/G1/G2 and `G_MS_*` constructors, including explicit `G1_le_real_projection` beside the pure MS endpoint `G_MS_sim`), `GameAdvantage.ec` (projected `game_pr`, definitional `Adv`, and the explicit middle-hop arithmetic `Adv_G1_MS_to_LE`), `GameMSHops.ec` (MS1..MS3c transition axioms/lemmas + composed `A_G0_to_G1_ms_transition_bound`), and `GameLEBridge.ec` (definitional LE projection lemmas on `G1_le_real_projection` / `G2_full_sim`, plus `A_G1_to_G2_le_transition_bound`). `Games.ec` is a thin facade importing those split modules; theorem-facing names remain unchanged, `G_MS_sim` remains the pure MS endpoint, and the top theorem now carries the intermediate-hop obligation explicitly rather than as a game-layer axiom.
