...
Docs checkpoint, May 2026: `./check_easycrypt.sh` remains `OK` over 149 checked theories, `axiom_count=0`, and `admit_count=0`. The theorem-surface corollary phase is complete in `theorem/MainTheorem.ec` through `qssm_main_theorem`, `qssm_main_theorem_semantic_budget`, `qssm_main_theorem_semantic_budget_umbrella`, and `qssm_main_theorem_nonzero_budget`, so no separate `Corollaries.ec` file is needed. `BudgetParameters.ec` and `theorem/MainTheorem.ec` stay unchanged on this docs checkpoint, the semantic top remains `3%r / 4%r`, and the exact-zero route remains unchanged.

Parameterization checkpoint, May 2026: the LE parameterized lane still closes through `GameLEBridgeParameterized.ec` into `theorem/MainTheoremParameterized.ec : qssm_main_theorem_le_parameterized_budget`, and that theorem still serves as the LE-only intermediate route that keeps the canonical/demo MS contribution unchanged while parameterizing only the LE side. The full canonical parameterized route now also closes in `theorem/MainTheoremParameterized.ec : qssm_main_theorem_parameterized_budget`. The MS public AfterRom endpoint is still only budget-close to the canonical AfterRom endpoint, not zero-equal, so the closure is budgeted rather than exact and explicitly pays `epsilon_ms_hash_binding_parameterized + epsilon_ms_rom_programmability_parameterized + epsilon_ms_rom_programmability_parameterized + epsilon_le_parameterized`, which evaluates to `15%r / 64%r` on the active live profiles. Since that checkpoint, both parameterized LE components have moved onto parallel live lower-budget lanes: `LERejectionSamplerParameterizedCore.ec` owns the parameterized rejection branch/marginal core, `LERejectionSamplerMassLiveParameterized.ec` owns the live parameterized rejection mass and `sdist` facts, `LEFsProgrammingLiveParameterizedCore.ec` owns the live parameterized FS branch/midpoint core, `LEFsProgrammingLiveParameterizedMass.ec` owns the live parameterized FS bad-branch mass and `sdist` facts, and `LEFsProgrammingParameterizedView.ec` plus `LEFsProgrammingParameterized.ec` retarget the FS bridge to that lane. The active parameterized rejection profile is now `soft=1`, `hard=1`, `invalid=1`, `accept=61`, `failure=3`, `total=64`, so `epsilon_le_rej_parameterized = 3%r / 64%r`; the active parameterized FS profile is `query_collision=1`, `programming_collision=1`, `transcript=1`, `clean=61`, `failure=3`, `total=64`, so `epsilon_le_fs_parameterized = 3%r / 64%r`; therefore `epsilon_le_parameterized = 3%r / 32%r`. Those LE retunings were owner-only and required no local proof repairs. The parameterized MS1 route is now also live below the theorem-facing wrappers: `SourceHashBindingSemanticLiveParameterizedCore.ec` owns the coupled-state/public-observable core, `SourceHashBindingSemanticLiveParameterizedMass.ec` owns canonical failure and public-divergence upper mass closure, and `SourceHashBindingSemanticBridgeParameterized.ec`, `MSProbabilitySurfaceParameterized.ec`, `GameAdvantageParameterized.ec`, `GameMSHopTypesParameterized.ec`, and `GameMSHopCompositionParameterized.ec` now carry the live route with `epsilon_ms_hash_binding_parameterized = 3%r / 64%r` and staged public-divergence upper mass `2%r / 64%r = 1%r / 32%r`. That MS1 retuning was owner-only and required no local proof repair. The parameterized MS2 route is now also live below the theorem-facing wrappers: `ComparisonPayloadSemanticLiveParameterizedCore.ec` owns the category/coupled-state/public-AfterRom core, `ComparisonPayloadSemanticLiveParameterizedMass.ec` owns execution-owned failure and public-divergence/failure mass closure, and `ComparisonPayloadSemanticBridgeParameterized.ec`, `MSProbabilitySurfaceParameterized.ec`, `GameAdvantageParameterized.ec`, `GameMSHopTypesParameterized.ec`, and `GameMSHopCompositionParameterized.ec` now carry the live route with `epsilon_ms_rom_programmability_parameterized = 3%r / 64%r`. That MS2 retuning was owner-only and required no local proof repair. The demo semantic route and exact-zero route remain unchanged, the duplicated MS2 charge remains explicit, and any future work here is broader profile generalization or stop rather than another localized replay slice unless sampler-internal weighted semantics become a hard requirement.

Real-world abstract-budget checkpoint, May 2026: `RealWorldBudgetParameters.ec`, `RealWorldBudgetObligations.ec`, `LEStatisticalDistanceRealWorld.ec`, `MSProbabilitySurfaceRealWorld.ec`, `GameLEBridgeRealWorld.ec`, `GameMSHopCompositionRealWorld.ec`, and `theorem/MainTheoremRealWorld.ec` now add `qssm_main_theorem_realworld_budget` as a parallel axiom-free upper-bound theorem surface. It consumes explicit `le_realworld_obligations`, `ms_realworld_obligations`, and `qssm_realworld_obligations` hypotheses rather than axioms, keeps the duplicated MS2 charge explicit, preserves the public AfterRom caveat, and does not implement weighted/non-uniform sampler semantics. That theorem surface is already sufficient when sampler-internal evidence stays external to the EasyCrypt tree.

Concrete external-bound instantiation checkpoint, May 2026: `RealWorldBudgetInstantiation.ec` now packages `realworld_budget_concrete_256`, proves the component budget `1 / 2^226` and the top closed form `5 / 2^226`, keeps `qssm_main_theorem_realworld_concrete_256` and `qssm_main_theorem_realworld_concrete_256_5_over_2_226` as the original concrete external-bound companions to `qssm_main_theorem_realworld_budget`, and now also adds additive sibling theorems ending at `qssm_main_theorem_realworld_concrete_256_with_all_reductions` and `qssm_main_theorem_realworld_concrete_256_with_all_reductions_5_over_2_226`. The fully reduction-facing sibling route is carried by `LERejectionConcreteReduction.ec`, `LEFsConcreteReduction.ec`, `LEConcreteReduction.ec`, `MS1ConcreteReduction.ec`, `MS2ConcreteReduction.ec`, and `MSConcreteReduction.ec`. These concrete theorems keep the duplicated MS2 charge explicit, preserve the public AfterRom caveat, use explicit premises rather than axioms, and do not introduce weighted/non-uniform sampler semantics.

Weighted replay audit checkpoint, May 2026: weighted finite-support replay remains future work only if the repository must model weighted sampler internals directly. If that work is ever approved, the preferred owner shape is normalized per-component category weights; per-slot weights are not the right first move; component-failure-only records are too abstract because they mostly duplicate the current obligation bundle; and the first safe pilot is an LE rejection weighted category owner only. That future work would discharge `le_realworld_obligations` below the existing real-world theorem surface rather than replace `qssm_main_theorem_realworld_budget`.
