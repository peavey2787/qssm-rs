...
- **Seed-support threading:** **`ComparisonPayloadFromSeed.ec`** now reads the payload-facing fields from the structured seed records, and support-local lemmas show that sampled seeds still collapse to the same native Phase-1 payload on support.
...
No new hook or axiom was added for the coupling redesign; the MS-3c assumption surface remains the six rows above.
...
At the hook-bridge layer, the remaining proved lemmas are now field-local: **`ComparisonCouplingAxioms.ec`** discharges the public/share fragments from per-side support witnesses and field-by-field **`from_seed`** equalities, so future payload-visible sampled-coin fields do not require the bridge layer to collapse real and sim payloads wholesale.
...
| **`L_ms3c_cross_support_real_sim_payload_equal`** | **lemma** (`ComparisonPayloadFromSeed.ec`) | On support, real and sim payloads are **`dmap`** preimages of the same Phase-1 **`ms3c_{real,sim}_payload_from_seed`** body ⇒ **`pr = ps`**. This legacy support-collapse lemma remains available in the payload layer but is no longer used by **`ComparisonCouplingAxioms.ec`**. |
...
**Still open:** Phase-1 comparison data and seed carriers are native, the component laws sample latent ROM/transcript scalar coins, the coupling layer no longer depends on collapsing paired payloads to the same support point, and the hook-bridge proofs no longer consume that singleton-support collapse either. The next MS-3c work is therefore lower in the stack: relax the current field-by-field Phase-1 support equalities in **`ComparisonPayloadFromSeed.ec`** / **`ComparisonPayloadSeedTypes.ec`** so sampled coins can drive payload-visible transcript/ROM fields while preserving the public/share bridge obligations. The **`A_ms3c_{real,sim}_from_seed_uses_*`** facts are now **support-local proved lemmas** in **`ComparisonPayloadSeedAnchors.ec`**, all four component samplers remain lossless, and the remaining proof debt is therefore in non-degenerate payload / execution modeling, not in placeholder or unit comparison carriers.
...
**Remaining design boundary:** the carrier plumbing is now native, the seed samplers now genuinely sample latent ROM/transcript coins, and the public game boundary is still closed because sampled seeds collapse to the same native Phase-1 payload on support. Future work is to keep that boundary fixed while allowing those sampled coins to affect payload-visible transcript state.
...
| **Public-field fragments ×4** | Phase-1: all four are proved in **`ComparisonCouplingAxioms.ec`** from per-side support witnesses and field-by-field **`from_seed`** equalities; ROM query digest on Phase-1 payloads is **`A_ms3c_clause_surface_query_digest_constructed`** (**proved**). |
| **Challenge-share fragments ×3** | Phase-1: all three are proved by the same field-local support-witness pattern; if **`from_seed`** diverges per side on compared share fields, refresh those fragment lemmas rather than reinstating **`pr = ps`**. |
...
| Risk | Mitigation |
|------|------------|
| **MS-3b carrier is concrete but payload-visible seed projections are still support-collapsed** | Keep the native comparison slice / opening carrier stable, and let the newly sampled latent ROM/transcript coins influence payload-visible state in a way the coupling layer can still prove. |
...
3. **Constructors (`ComparisonPayloadFromSeed.ec` / `ComparisonPayloadSeedTypes.ec`):** partially done — **`ms3c_real_payload_from_seed`** and **`ms3c_sim_payload_from_seed`** now read the structured seed records, the component laws now sample latent coins via **`dmap duni_scalar`**, and the shape lemmas in **`ComparisonPayloadSeedAnchors.ec`** are now support-local. Remaining work is to expose sampled variation in payload-visible fields rather than collapsing back to the same public Phase-1 image on support.
