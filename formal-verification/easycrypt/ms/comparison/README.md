...
- `ComparisonPayloadSemanticLiveParameterizedMass.ec` owns the live parameterized MS2 execution-owned failure mass, public-divergence/failure comparison, and budget closure at `epsilon_ms_rom_programmability_parameterized = 3%r / 64%r`.
- That final MS2 retuning was owner-only after the live lower lane was installed, changed no theorem-surface names, and required no local proof repair.
