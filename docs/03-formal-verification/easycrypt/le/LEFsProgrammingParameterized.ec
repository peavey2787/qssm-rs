...
have Heq :
    BudgetParameters.epsilon_le_fs_semantic =
    le_fs_failure_probability_parameterized x s.
  rewrite epsilon_le_fs_semantic_eq_epsilon_le_fs_parameterized.
  rewrite -(le_fs_failure_probability_eq_epsilon_le_fs_parameterized x s).
  by [].
rewrite -Heq.
exact LEFsProgrammingSurface.le_fs_shadow_local_bad_branch_mass_le_epsilon_le_fs_semantic.
qed.
