...
- A parallel theorem-facing semantic LE chain is now present: `A_LE_fs_semantic_programming_sampler_sdist_le_bad_branch_mass`, `A_LE_semantic_combined_hiding_bounds_sdist`, `A_LE_HVZK_semantic_transition_bound`, and `qssm_main_theorem_semantic_budget`. The semantic view chain ends in `epsilon_le_rej + le_fs_shadow_local_bad_branch_mass`, while the top-level QSSM theorem exposes the conservative additive bound `epsilon_ms_hash_binding + epsilon_ms_rom_programmability + epsilon_le_rej + le_fs_shadow_local_bad_branch_mass`.
...
- The next meaningful refinement is to move from the current parallel semantic theorem chain to a fully budget-owned nonzero FS component in `BudgetParameters.ec` and then relax the active game bridge away from the exact-zero `epsilon_le` endpoint.