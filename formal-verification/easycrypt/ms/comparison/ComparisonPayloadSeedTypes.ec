...
(* Auxiliary transcript view for the comparison-local execution-seed lane: the
  sampled transcript coin parameterizes a non-public comparison view without
  changing the compared announcements. The packaged execution carrier now lives
  in `ComparisonPayloadExecutionSeedTypes.ec`, while this file retains the
  component and payload-seed surface consumed by the theorem-facing payload
  layer. *)
