...
| **`A_ms3c_{real,sim}_payload_support_length_index_shapes`** | **lemma** | On-support payloads satisfy **`ms3c_payload_length_index_shapes_ok`**, proved from `supp_dmap` + both share-length lemmata + both public-index anchors (via index-shape lemmata). |
...
| **`A_ms3c_surface_query_digest_field_correct`** | **`stmt := ms3c_public_stmt_digest x`** (or **`ms_statement_digest obs`** with **`ms3a_frame_consistent`**-style alignment); **`mscp_query_digest`** field set to **`ms_comparison_query_digest stmt (ms3c_clause_ann_digests_from_surface c)`** from observable-carried announcements. |
...
| **False-index nonempty** (**`A_ms3c_{real,sim}_constructor_false_index_nonempty`**) | **`0 < ms3c_public_false_branch_count x`** (or explicit predicate **`ms3c_comparison_nontrivial x`**) on **`ms_public_input`**. |
...
This section is a **design-only** blueprint to discharge the **15** remaining **`A_ms3c_*`** axioms in **`ms/comparison/`** (shape, digest surface, coupling fragments, false-clause hooks) without changing existing **theorem statements** (only definitions and proofs behind abstract ops). It aligns **`ComparisonPayloadTypes.ec`**, **`ComparisonPayloadSeeds.ec`**, and later **game hops** with **`ms_transcript_observable`** / **`ms_v2_transcript_observable`** (`ms/TranscriptObservable.ec`, `ms/SourceModel.ec`).
...
| **Seed shape** | Real index-shape: **proved** from anchor + public projections; remaining length + sim index: **`ms3c_*_payload_from_seed`** + list arithmetic from **`x`**. |
