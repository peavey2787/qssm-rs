...
require export ComparisonPayloadSemanticSlotMass.
...
lemma ms_rom_public_observable_divergence_mass_category_choiceE
  (x : ms_public_input) :
  ms_rom_public_observable_divergence_mass x =
  mu1 (dmap d_ms_rom_semantic_category_choice
    (ms_rom_public_divergence_flag_of_category x)) true.
proof.
have Hmu1 :
    mu (dmap (d_ms_rom_semantic_coupled_state x)
      (ms_rom_public_observable_divergence_condition x))
      (fun bad : bool => bad) =
    mu1 (dmap (d_ms_rom_semantic_coupled_state x)
      (ms_rom_public_observable_divergence_condition x)) true.
  apply/mu_eq=> bad /=.
  by case: bad.
have Hmap :
    dmap (d_ms_rom_semantic_coupled_state x)
      (ms_rom_public_observable_divergence_condition x) =
    dmap d_ms_rom_semantic_category_choice
      (ms_rom_public_divergence_flag_of_category x).
  rewrite /d_ms_rom_semantic_coupled_state.
  rewrite (dmap_comp
    (fun (p : ms3c_real_execution_seed * BudgetParameters.ms_rom_semantic_category) =>
       ms_rom_semantic_state_of_category_execution_seed x (fst p) (snd p))
  ...
  have Hmap_prod :
      dmap ((d_ms3c_real_execution_seed x) `*` d_ms_rom_semantic_category_choice)
        (ms_rom_public_observable_divergence_condition x \o
          (fun (p : ms3c_real_execution_seed * BudgetParameters.ms_rom_semantic_category) =>
             ms_rom_semantic_state_of_category_execution_seed x (fst p) (snd p))) =
      dmap ((d_ms3c_real_execution_seed x) `*` d_ms_rom_semantic_category_choice)
        (fun (p : ms3c_real_execution_seed * BudgetParameters.ms_rom_semantic_category) =>
           ms_rom_public_divergence_flag_of_category x (snd p)).
    apply eq_dmap_in=> p Hp /=.
    rewrite /(\o).
    rewrite supp_dprod in Hp.
    move: Hp=> [Hsigma _].
    exact (ms_rom_public_observable_divergence_condition_of_categoryE x (fst p) (snd p) Hsigma).
  rewrite Hmap_prod.
  rewrite -(dmap_comp snd (ms_rom_public_divergence_flag_of_category x)
    ((d_ms3c_real_execution_seed x) `*` d_ms_rom_semantic_category_choice)).
  have Hsnd :
...
rewrite dmapE /= in Hmu1.
rewrite Hmu1.
by rewrite Hmap.
qed.

lemma ms_rom_public_observable_divergence_visible_slot_countE
  (x : ms_public_input) :
  mu1 (dmap d_ms_rom_semantic_category_choice
    (ms_rom_public_divergence_flag_of_category x)) true =
...
proof.
rewrite /mu1 /d_ms_rom_semantic_category_choice.
rewrite (dmap_comp ms_rom_semantic_category_of_slot
  (ms_rom_public_divergence_flag_of_category x)
  d_ms_rom_semantic_slot_choice).
have Hmap :
    dmap d_ms_rom_semantic_slot_choice
      (ms_rom_public_divergence_flag_of_category x \o ms_rom_semantic_category_of_slot) =
    dmap d_ms_rom_semantic_slot_choice
      (fun (slot : int) =>
        if slot < BudgetParameters.ms_rom_query_collision_slot_count then
          ms_rom_public_divergence_global_digest_flag x
  ...
  - have Hquery_slotE :
        (slot < BudgetParameters.ms_rom_query_collision_slot_count) = true by smt().
    by rewrite /(\o) /ms_rom_public_divergence_flag_of_category
      /ms_rom_semantic_category_of_slot Hquery_slotE /=.
  have Hquery_slotE :
      (slot < BudgetParameters.ms_rom_query_collision_slot_count) = false by smt().
  have [Hprog_slot|Hprog_slot] :
      slot <
  ...
  - have Hprog_slotE :
        (slot <
          ...
          BudgetParameters.ms_rom_programming_collision_slot_count) = true by smt().
    by rewrite /(\o) /ms_rom_public_divergence_flag_of_category
      /ms_rom_semantic_category_of_slot Hquery_slotE Hprog_slotE /=.
  have Hprog_slotE :
      (slot <
        BudgetParameters.ms_rom_query_collision_slot_count +
        BudgetParameters.ms_rom_programming_collision_slot_count) = false by smt().
  ...
  - have Hmismatch_slotE :
        (slot < BudgetParameters.ms_rom_failure_slot_count) = true by smt().
    by rewrite /(\o) /ms_rom_public_divergence_flag_of_category
      /ms_rom_semantic_category_of_slot Hquery_slotE Hprog_slotE Hmismatch_slotE /=.
  have Hmismatch_slotE :
      (slot < BudgetParameters.ms_rom_failure_slot_count) = false by smt().
  by rewrite /(\o) /ms_rom_public_divergence_flag_of_category
    /ms_rom_semantic_category_of_slot Hquery_slotE Hprog_slotE Hmismatch_slotE /=.
...
lemma ms_rom_public_observable_divergence_mass_flagsE
  (x : ms_public_input) :
  ms_rom_public_observable_divergence_mass x =
  ((if ms_rom_public_divergence_global_digest_flag x then
      (BudgetParameters.ms_rom_query_collision_slot_count +
...
lemma ms_rom_execution_owned_semantic_failure_probability_eq_visible_plus_silent_failure_mass
  (x : ms_public_input) :
  ms_rom_execution_owned_semantic_failure_probability x =
  ms_rom_public_observable_divergence_mass x +
  ms_rom_public_silent_failure_mass x.
...
lemma ms_rom_execution_owned_visible_silent_fully_silentE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = false =>
  ms_rom_public_divergence_query_digest_flag x = false =>
  ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hglobal Hquery; split.
- exact (ms_rom_execution_owned_semantic_failure_probability_eq_visible_plus_silent_failure_mass x).
exact (ms_rom_public_visible_silent_fully_silentE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_query_visible_onlyE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = false =>
  ms_rom_public_divergence_query_digest_flag x = true =>
  ms_rom_execution_owned_semantic_failure_probability x =
...
move=> Hglobal Hquery; split.
- exact (ms_rom_execution_owned_semantic_failure_probability_eq_visible_plus_silent_failure_mass x).
exact (ms_rom_public_visible_silent_query_visible_onlyE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_global_visible_onlyE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = true =>
  ms_rom_public_divergence_query_digest_flag x = false =>
  ms_rom_execution_owned_semantic_failure_probability x =
...
move=> Hglobal Hquery; split.
- exact (ms_rom_execution_owned_semantic_failure_probability_eq_visible_plus_silent_failure_mass x).
exact (ms_rom_public_visible_silent_global_visible_onlyE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_fully_visibleE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = true =>
  ms_rom_public_divergence_query_digest_flag x = true =>
  ms_rom_execution_owned_semantic_failure_probability x =
...
lemma ms_rom_execution_owned_visible_silent_casesE
  (x : ms_public_input) :
  (ms_rom_public_divergence_global_digest_flag x = false /\
   ms_rom_public_divergence_query_digest_flag x = false /\
   ((ms_rom_execution_owned_semantic_failure_probability x =
  ...
  (ms_rom_public_divergence_global_digest_flag x = true /\
   ...
   ((ms_rom_execution_owned_semantic_failure_probability x =
    ...
    ms_rom_execution_owned_semantic_failure_probability x =
      BudgetParameters.epsilon_ms_rom_programmability_semantic)).
proof.
have [Hglobal|Hglobal] :
    ms_rom_public_divergence_global_digest_flag x = false \/
    ms_rom_public_divergence_global_digest_flag x = true by smt().
- have [Hquery|Hquery] :
      ms_rom_public_divergence_query_digest_flag x = false \/
      ms_rom_public_divergence_query_digest_flag x = true by smt().
  + left; split; [exact Hglobal | split; [exact Hquery | exact (ms_rom_execution_owned_fully_silent_bundleE x Hglobal Hquery)]].
  + right; left; split; [exact Hglobal | split; [exact Hquery | exact (ms_rom_execution_owned_query_visible_only_bundleE x Hglobal Hquery)]].
- have [Hquery|Hquery] :
      ms_rom_public_divergence_query_digest_flag x = false \/
      ms_rom_public_divergence_query_digest_flag x = true by smt().
  + right; right; left; split; [exact Hglobal | split; [exact Hquery | exact (ms_rom_execution_owned_global_visible_only_bundleE x Hglobal Hquery)]].
  + right; right; right; split; [exact Hglobal | split; [exact Hquery | exact (ms_rom_execution_owned_fully_visible_bundleE x Hglobal Hquery)]].
qed.

lemma ms_rom_execution_owned_visible_silent_cases_fully_silentE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = false =>
  ms_rom_public_divergence_query_digest_flag x = false =>
  (ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hglobal Hquery.
exact (ms_rom_execution_owned_fully_silent_bundleE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_cases_query_visible_onlyE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = false =>
  ms_rom_public_divergence_query_digest_flag x = true =>
  (ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hglobal Hquery.
exact (ms_rom_execution_owned_query_visible_only_bundleE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_cases_global_visible_onlyE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = true =>
  ms_rom_public_divergence_query_digest_flag x = false =>
  (ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hglobal Hquery.
exact (ms_rom_execution_owned_global_visible_only_bundleE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_cases_fully_visibleE
  (x : ms_public_input) :
  ms_rom_public_divergence_global_digest_flag x = true =>
  ms_rom_public_divergence_query_digest_flag x = true =>
  (ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hglobal Hquery.
exact (ms_rom_execution_owned_fully_visible_bundleE x Hglobal Hquery).
qed.

lemma ms_rom_execution_owned_visible_silent_common_equalitiesE
  (x : ms_public_input) :
  ms_rom_execution_owned_semantic_failure_probability x =
    ms_rom_local_failure_mass /\
  ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
split.
- exact (ms_rom_execution_owned_semantic_failure_probability_eq_local_mass x).
rewrite (ms_rom_execution_owned_semantic_failure_probability_eq_local_mass x).
exact ms_rom_local_failure_mass_eq_epsilon_ms_rom_programmability_semantic.
qed.
...
lemma ms_rom_execution_owned_visible_silent_common_equalities_from_casesE
  (x : ms_public_input) :
  (ms_rom_execution_owned_semantic_failure_probability x =
     ms_rom_local_failure_mass /\
   ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
move=> Hcases.
have Hequiv := ms_rom_execution_owned_visible_silent_common_equalities_equivE x.
by smt().
qed.

lemma ms_rom_execution_owned_visible_silent_cases_common_equalities_from_directE
  (x : ms_public_input) :
  (ms_rom_execution_owned_semantic_failure_probability x =
     ms_rom_local_failure_mass /\
   ms_rom_execution_owned_semantic_failure_probability x =
...
lemma ms_rom_execution_owned_visible_silent_normalized_bundleE
  (x : ms_public_input) :
  ms_rom_execution_owned_semantic_failure_probability x =
    ms_rom_local_failure_mass /\
  ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
split.
- exact (ms_rom_execution_owned_visible_silent_normalized_local_massE x).
exact (ms_rom_execution_owned_visible_silent_normalized_budgetE x).
qed.

lemma ms_rom_execution_owned_visible_silent_normalized_equivE
  (x : ms_public_input) :
  (ms_rom_execution_owned_semantic_failure_probability x =
     ms_rom_local_failure_mass /\
   ms_rom_execution_owned_semantic_failure_probability x =
...
proof.
split.
- exact (ms_rom_execution_owned_visible_silent_common_equalities_normalizeE x).
...
lemma ms_rom_execution_owned_visible_silent_normalized_equiv_roundtripE
  (x : ms_public_input) :
  (ms_rom_execution_owned_semantic_failure_probability x =
     ms_rom_local_failure_mass /\
   ms_rom_execution_owned_semantic_failure_probability x =
  ...
  (ms_rom_execution_owned_semantic_failure_probability x =
   ...
   ms_rom_execution_owned_semantic_failure_probability x =
     BudgetParameters.epsilon_ms_rom_programmability_semantic).
proof.
have Hequiv := ms_rom_execution_owned_visible_silent_normalized_equivE x.
have Hcommon := ms_rom_execution_owned_visible_silent_normalized_equiv_to_commonE x.
have Hbundle := ms_rom_execution_owned_visible_silent_normalized_equiv_to_bundleE x.
by smt().
qed.
...
qed.
  ...
  lemma ms_rom_public_observable_divergence_mass_le_local_failure_mass
    (x : ms_public_input) :
    ms_rom_public_observable_divergence_mass x <=
    ms_rom_local_failure_mass.
  proof.
  have Hmass :=
    ms_rom_public_observable_divergence_mass_le_execution_owned_semantic_failure x.
  rewrite ms_rom_execution_owned_semantic_failure_probability_eq_local_mass in Hmass.
  exact Hmass.
  qed.
