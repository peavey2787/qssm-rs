...
- **Theorem:** `theorem/MainTheorem.ec` — remaining non-negativity placeholders `A1_ms_hash_binding` and `A2_ms_rom_programmability`, direct use of the primitive LE budget assumption `A4_le_hvzk_bound_nonneg` from `LESurface.ec`, bridge lemmas `use_MS_3a` / `use_MS_3b` / `use_MS_3c`, and proved additive game-hop lemma `qssm_main_theorem_skeleton`, now specialized to `extract_ms_public x` rather than a free unrelated `xms`
