...
- Two parallel semantic LE chains are now present. The local-mass chain still ends in `epsilon_le_rej + le_fs_shadow_local_bad_branch_mass` and feeds `qssm_main_theorem_semantic_budget`. The owned-budget chain ends in `epsilon_le_rej + epsilon_le_fs_semantic` and feeds `A_G1_to_G2_le_semantic_owned_budget_transition_bound` plus `qssm_main_theorem_semantic_budget_owned`.
...
- The next meaningful refinement is to decide whether the owned semantic bridge should remain a parallel theorem path or become the active exported LE budget path, and then whether `epsilon_le` itself should gain a parallel semantic-owned analogue rather than staying tied to the exact-zero route.