...
pub fn words_from_little_endian_bytes(bytes: &[u8], words: &mut [u32]) {
    // SAFETY: Called only from `MerkleParentHashWitness::build` with hardcoded 64-byte
    // slices into 16-word arrays (64 == 4 * 16). Both call sites are structurally
    // constrained to matching sizes. `debug_assert` fires in test builds as defense-in-depth.
    debug_assert!(
        bytes.len() == 4 * words.len(),
        "BLAKE3: byte slice length {} is not 4 * word count {}",
        bytes.len(),
        words.len(),
    );
    for (four, w) in bytes.chunks_exact(4).zip(words.iter_mut()) {
        // SAFETY: chunks_exact(4) guarantees exactly 4 bytes per chunk.
        *w = u32::from_le_bytes([four[0], four[1], four[2], four[3]]);
    }
}
...
pub fn first_8_words(compression_output: [u32; 16]) -> [u32; 8] {
    // Infallible: source is always [u32; 16], so [0..8] is always valid.
    [
...
impl MerkleParentHashWitness {
    ...
    pub fn build(left: &[u8; 32], right: &[u8; 32]) -> Self {
        ...
        // SAFETY: `preimage` is constructed from DOMAIN_MERKLE_PARENT (fixed-length)
        // + left (32 B) + right (32 B) immediately above. The length is structurally
        // guaranteed. `debug_assert` fires in test builds as defense-in-depth.
        debug_assert!(
            preimage.len() == DOMAIN_MERKLE_PARENT.len() + 64,
            "merkle parent preimage is domain + 64 B (got {})",
            preimage.len(),
        );
